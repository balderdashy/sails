...
options {}
	...
	caseConversion :: a function that takes the module identity and converts it to the desired case. Defaults to lower case.
*/
function buildDictionary(options) {
	...
	if (!options.caseConversion) {
		options.caseConversion = function (identity) {
		...
		// Build the module dictionary
			...
			if (!options.dontLoad && options.identity !== false) {
				...
				if(!module.identity) {
					...
					module.identity = options.caseConversion(module.identity);
