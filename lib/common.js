...
exports.lift = function liftSails (appPath) {
	loadSails(appPath);
	initSails(appPath);
}

// Load the dependencies and app-specific components
exports.load = function loadSails (appPath) {
	...
	// Configuration
	if (! path.existsSync(appPath+'/config/db.js')) {
		throw new Error ('No database configuration file found!');
	}
	db = require(appPath+'/config/db'); 
	config = require(appPath+'/config/app');
	require(appPath+'/config/log');
	...
	*/
	...
	app.configure(function() {
		...
		{
			app.use(express['static'](appPath+'/mast'));
...
exports.initialize = function initSails (appPath){
	...
	if (rigging) {
		require(__dirname + '/rigging/compiler').compile(startServers);
	}
	else {
		startServers();
	}
