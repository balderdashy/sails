...
exports.load = function loadSails(userConfig, cb) {
	...
	db.sync(function() {
		...
		// Start HTTP server
		if(_.isUndefined(config.serverOptions)) app = express.createServer();
		else app = express.createServer(config.serverOptions);
...
exports.initialize = function initSails(userConfig) {
	...
	function startServers() {
		...
		if(!app.address()) {
			...
			debug.warn('*************', 'Usually this means you have an instance of Sails, or something else, running on this port.', '*************');
		} else {
