...
exports.load = function loadSails(userConfig, cb) {
	...
	// Load rigging library
	sails.rigging = require('rigging');
	...
	// Determine rigging path (defaults to /public/ui)
	// This is where assets go that will be compiled
	config.riggingPath = config.riggingPath || config.appPath + "/public/ui";

	// Determine rigging sequence (defaults to the following)
	config.riggingSequence = config.riggingSequence || 
		[config.appPath+"/node_modules/mast/mast.min.js", config.riggingPath];
...
function afterModelInitialization() {
	...
	app.configure(function() {
		...
		if(config.environment === 'development') {
			...
			app.use(express['static'](config.riggingPath));

			// (legacy) allow direct browser access to Mast core and files
...
exports.initialize = function initSails(userConfig) {
	...
	sails.rigging.compile(config.riggingSequence, {
