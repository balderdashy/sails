...
exports.load = function loadSails(userConfig, cb) {
	...
	// Get app models and adapters
	// (default waterline adapters are included automatically)
	sails.models = require("./models");	
	sails.adapters = require("./adapters");
...
exports.initialize = function initSails(userConfig,cb) {
	...
	sails.log.info("Initializing rigging middleware...");
		rigging.compile(sails.config.rigging.sequence, {
			...
			if (err) {
				sails.log("***** Unable to compile assets!! *****");
				sails.log.error(err);
				startServers();
			}
			else startServers();
			...
			if(data.headers.cookie) {
				...
				sails.config.session.store.get(data.sessionID, function(err, session) {
					...
					// Cookie is invalid, so regenerate a new one
						...
						_.shout("Generated new session....",data);
						accept(null, true);
					...
					// Save the session handshake and accept the connection
						...
						sails.log("Connected to existing session....");
						// _.shout("Connected to existing session....",data);
						accept(null, true);
					}
				});
			} else {
