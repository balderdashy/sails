...
exports.load = function loadSails(userConfig, cb) {
	...
	var instantiatedModules = $$(waterline)({
	...
	$$(function afterWaterlineInstantiated (err, instantiatedModules,xcb){
		...
		// Run boostrap script
		if (sails.config.bootstrap) sails.config.bootstrap(xcb);
		else xcb();
	})(instantiatedModules);


	$$(function afterBootstrap (xcb) {
