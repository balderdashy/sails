...
var JavascriptAsset = rack.Asset.extend({
    ...
    createProduction: function() {
        this.pathInfo = search(sails.config.rigging.sequence, /\.js$/);
        this.url = '/rigging_production/rigging.min.js';
        this.contents = '';
        
        _.each(this.pathInfo, function(path) {
            this.contents += fs.readFileSync(path.path, 'utf8');
        })        
    ...
    createDev: function() {
        ...
        this.pathInfo = search(sails.config.rigging.sequence, /\.js$/);
        this.assets = []
        async.forEachSeries(this.pathInfo, function(path, next) {
            asset = new rack.Asset({
                mimetype: 'text/javascript',
                url: '/' + pathutil.relative(path.dirname, path.path),
                contents: fs.readFileSync(path.path, 'utf8')
            });
...
var CssAsset = rack.Asset.extend({
    ...
    createDev: function() {
        var self = this;
        this.pathInfo = search(sails.config.rigging.sequence, this.regex);
        this.assets = []
        async.forEachSeries(this.pathInfo, function(path, next) {
            var asset;
            if (pathutil.extname(path) == '.less') {
                asset = new rack.LessAsset({
                    url: '/' + pathutil.basename(path).replace(/\.less/, '.css'),
                    filename: path
                });
            } else {
                asset = new rack.Asset({
