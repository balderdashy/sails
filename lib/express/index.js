module.exports = function (sails) {
	...
	return function loadExpress (cb) {
		...
		// Limit file uploads per route
		// pre-calculate regexp for performance
		var limitedRoutes = [];
		for (var key in sails.config.routes) {
			var route = sails.config.routes[key];
			if (typeof route === 'object' && route.uploadLimit) {
				limitedRoutes.push({matcher: expresstUtils.pathRegexp(key), limit: route.uploadLimit});	
			}
		}

		app.use(function(req, res, next) {

			// Limit upload based on route
			for (var key in limitedRoutes){
				var match = limitedRoutes[key].matcher.exec(req.url);
				if(match) {
					return express.limit(limitedRoutes[key].limit)(req, res, next);
				}
				
			}
			
			// default to a limit of 10mb
			return express.limit('10mb')(req, res, next);
		});
		...
		// Use CORES if set per route
		var coresRoutes = [];
		for (var key in sails.config.routes) {
			var route = sails.config.routes[key];
			if (typeof route === 'object' && route.cores) {
			  	if (typeof route.cores === 'boolean') {
      				route.cores = '*';
				}
				coresRoutes.push({matcher: expresstUtils.pathRegexp(key), origin: route.cores});	
			}
		}

		app.use(function(req, res, next) {

			// Set CORES header based on route
			for (var key in coresRoutes){
				var match = coresRoutes[key].matcher.exec(req.url);
				if(match) {
				  res.setHeader('Access-Control-Allow-Origin', coresRoutes[key].origin);
				  return next();
				}
