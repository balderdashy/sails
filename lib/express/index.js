module.exports = function (sails) {
	...
	return function loadExpress (cb) {
		...
		if (sails.config.cors.enabled){

			var config = sails.config.cors;
			sails.log.debug('CORS Enabled..');

			app.use(function(req, res, next){
				// implode arrays to comma delimited, just in case
			  config.allowedMethods.join(',');
			  config.allowedDomains.join(',');

			  // setup the three headers
