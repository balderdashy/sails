	...
	var actions = {
		...
		find: function(req, res) {
			...
			Model.find(req.param('id')).done(function(err, model) {
				...
				// Be flexible as to what the model might return
				// to support different sort of adapters and be futur
				var values;
				...
				else return formatResponse(Model, values, res);
		...
		findAll: function(req, res) {
			...
			finding.done(function afterFound(err, models) {
				...
				// Otherwise serve a JSON API
				else return formatResponse(Model, modelValues, res);
			});
		},
		...
		create: function(req, res) {
			...
			// Otherwise create model and serve JSON
				...
				Model.create(params, function(err, model) {
				    ...
				    return formatResponse(Model, values, res);
					}
				});
...
function formatResponse(Model, data, res) {
  if (sails.config.apiAdapter === 'ember') {
    // Ember friendly response
    var d = {};
    d[pluralize(Model.identity)] = data;
    return res.json(d);
  }
  // return raw response
...
// pluralise the model name. More rules might well be needed
