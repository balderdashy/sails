...
function liftSails(configOverride, cb) {
	...
	async.auto({
		...
		sails.log('Sails lifted on port ' + _.result(sails.config,"port") + ' in ' + sails.config.environment + ' mode.');
		...
		if (sails.config.environment === 'development') {
			...
			sails.log();
			sails.log('( to see your app, visit: ' + ( usingSSL ? 'https' : 'http' ) + '://' + sails.config.host + ':' + _.result(sails.config, "port") + ' )');
		}
