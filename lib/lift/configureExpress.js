...
module.exports = function configure(cb) {
	...
	app.configure(function(){
		...
		// When Grunt notices changes to `assets, it notifies Sails,
		// so tht Sails can provide access to assets as a view partial.
		// Initially, Sails discovers all scripts in `assets/js/**/*`
		console.log(sails.config.appPath + '/assets/js/**/*.js');
		require('glob')(sails.config.appPath + '/assets/js/**/*.js', function (err, scripts) {
			if (err) {
				return sails.log.warn('Error loading client-side javascript files for use in `js()` view partial.');
			}
			_.each(scripts, function (scriptSrc) {
				console.log('->',scriptSrc, '---->',require('path').relative(sails.config.appPath, scriptSrc));
				require('path').relative(sails.config.appPath, scriptSrc);
			});
		});
