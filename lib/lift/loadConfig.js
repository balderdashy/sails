...
module.exports = function (cb) {
	...
	async.auto({
		...
		appConfig: function (cb) {
			...
			// Merge user config with defaults & the app package.json - name should really come from package rather than config but..
			var packageFields = ["version"];
			sails.config = _.extend(sails.config, _.pick(require(userConfig.appPath+"/package.json"), packageFields));
