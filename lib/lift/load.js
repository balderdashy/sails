module.exports = function (sails) {
	...
	function loadHooks (cb) {
		...
		_.each(sails.config.hooks, function (hookPrototype, id) {
			...
			sails.log.verbose('Instantiating hook: '+id);
		});


		console.log('WTF',sails.hooks);

		// Load all hooks
		async.each(_.keys(sails.hooks), function (hook, cb) {
			setTimeout(function () {
				console.log('AGH');
				cb();
			},2);
			// console.log('LOADING HOOK',hook.load);
			// hook.load(cb);
		}, function (err) {
			console.log('did it finish?!',err, sails.hooks);
			cb();
		});
	}
