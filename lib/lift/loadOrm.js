...
module.exports = function (cb) {
  ...
  // Mixin config from Sails with Defaults from Adapter
  _.each(sails.adapters, function(adapter, identity) {
    var config = adapter.config || {};
    var defaults = adapter.defaults || {};

    if(sails.config.adapters[identity]) {
      config = sails.config.adapters[identity];
    }

    _.extend(config, _.clone(defaults));
  ...
  Object.keys(sails.models).forEach(function(model) {
    ...
    // Check if Model has an adapter property defined
    if(!sails.models[model].adapter) return;

    var adapterName = sails.models[model].adapter;
    sails.log.verbose('Loading adapter for '+model, '(',adapterName,')');
    ...
    if(!exists) {
      sails.log.error('To use '+adapterName+ ', please run `npm install '+adapterName+'`');
      process.exit(1);
    }
