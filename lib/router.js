...
function enhanceRequest(req, res, controllerName, actionName) {
	...
	// Add isJson flag to request object
	req.isJson = req.header('content-type') === 'application/json' || req.header('Accept') === 'application/json';
...
function reroute(routePlan, req, res, next) {
	...
	// If a string was specified, lookup the appropriate policy
		...
		if (plan) plan(req,res,next);
			...
			var errorScaffold = require('./scaffolds/error');
			req.params.errors = ['Trying to use unknown policy ('+routePlan+')', 'Please check your configuration in /config/policies.js, and make sure any string policies you specify have a matching file in /api/policies.'];
			return errorScaffold(req,res);
		}
	}
