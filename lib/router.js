...
function startHandlingMappedRequests(bindRouteFn) {
	...
	_.each(sails.routes, function(target, urlExpr) {
		...
		verb = _.last(urlExpr.match() || []) || null;
		...
		var actualTarget = _.extend(fetchRoute(urlExpr),{verb: verb});
 ...
 * and return an entity/action object
 * If no match exists, return null
 */
var fetchRoute = exports.fetchRoute = function(url) {
		...
		var hopcount = 0;
		while(_.isString(target) && sails.routes[target]) {
			target = sails.routes[target];
