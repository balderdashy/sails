...
function handler(req, res, next) {
	...
	function afterwards() {
		...
		function afterThat(err, exists) {
			...
			// Override next() and trigger the scaffold, but only if it exists
...
function runPolicy(controllerName, actionName, req, res, next) {
	...
	var controller = sails.policies[controllerName];
	if(controller && (!_.isUndefined(controller[actionName]) || !_.isUndefined(controller['*']))) {
		...
		// Use app default
		routePlan = !_.isUndefined(sails.policies['*']) ? sails.policies['*'] : true;
	}
...
function reroute(routePlan, req, res, next) {
	...
	// If a string was specified, lookup the appropriate policy
		...
		else throw new Error('Trying to use unknown policy ('+routePlan+').  Please check your configuration in /config/policies.js, and make sure any string policies you specify have a matching file in /api/policies.');
	}
