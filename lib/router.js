...
var mappingConfig = require(appPath+'/config/mappings'),
...
function enhanceRequest (req,res,controllerName,actionName) {
	...
	res.view = function(path,data,fun){
		...
		// if this is an ajax or socket request, use the JSON view equivalent
		// and if extension is not specified, and it exists,
		if (req.isAjax || req.isSocket) {
			fs.stat(app.settings.views+"/"+path+'.json', function(err, stat) {
					...
					if (fileExists && path.split(".").length == 1) {
						...
						debug.debug("No JSON found, rendering standard view for AJAX or Socket request:",path,controllerName,actionName);
					}
...
var permissionConfig = require(appPath + '/config/permissions'),
