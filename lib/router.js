...
function startHandlingMappedRequests(bindRouteFn) {
	...
	_.each(sails.routes, function(target, urlExpr) {
		...
		if (!actualTarget) {
			_.shout("actualTarget",actualTarget);
			_.shout("urlExpr",urlExpr);
			_.shout("target",target);
			_.shout("sails.routes",sails.routes);
			throw new Error("!!!!!");
		}
...
/**
 * Given a clearn URL (@url) and a verb, combine them, 
...
var fetchRoute = exports.fetchRoute = function(url, verb) {
		...
		var hopcount = 0;
		while(_.isString(target) && (sails.routes[verb.toLowerCase() + " " + target] || sails.routes[target]) ) {
			// Try both a simple URL and the verb + URL
			target = sails.routes[verb.toLowerCase() + " " + target] || sails.routes[target];
...
var handleRequest = exports.handleRequest = function(target) {
		...
		return function(req, res, next) {
			...
			res.locals({
				rigging: _.objInvoke(require('./assets'))
			});
...
function handler(req, res, next) {
	...
	function afterwards() {
		...
		// If it's not valid
			...
			require('fs').stat(app.settings.views + "/" + requestedEntity + "/" + actionName + ".ejs", function(err, stat) {
				if(err === null) {
					fn = renderStaticView;
				}
	...
	if(controller && (!_.isUndefined(controller[actionName]) || !_.isUndefined(controller['*']))) {
		...
		if(!_.isUndefined(action)) {
			...
			// If an object was specified, use the policy key
				...
				// TODO: support parameter validation shortcuts
			}
			...
			// If false was specified, always deny
				...
				throw new Error("Invalid entry in policy configuration! Controller: " + controllerName + "\nAction: " + actionName);
			}
