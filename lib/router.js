...
exports.mapSocketRequests = function (app,io) {
	...
	io.sockets.on('connection', function(socket) {
		...
		socket.on('message', function(request,fn) {
			...
			var pieces = path.split('/');										// Split url and determine controller/action
			request = _.extend({												// Add controller & action to request data
				entity		: pieces[1],
				actionName	: pieces[2]
			},request);
			...
			var expressContext = socketInterpreter.interpret(request,fn,socket,app);
...
function processSocketRequest (controllerName,actionName) {		
	return function(req,res,app){
		...
		accessControlMiddleware(controllerName,actionName,req,res,function() {
			var controller = controllers[controllerName] || require('./scaffolds/controller').definition(controllerName);
