...
function guessAction(req) {
	...
	// If controller doesn't exist, return the action naively
	else if(!controller) {
		return requestedAction;
	}
	// if action exists in controller, return immediately
	else if (controller[requestedAction]) {
		return requestedAction;
	}
	// Otherwise, if requestedAction was specified, but doesn't exist in controller 
	else {
