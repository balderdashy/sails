...
// temp
exports.mapExpressRequests = function (app) {

// Listen for requests listed in the routing table
function routeMappedRequests() {

	// Map standard AJAX and REST routes
	_.each(urlMappings,function(route,url) {
		...
		if (_.isObject(route)) {
			listen(url,processExpressRequest(route.controller,route.action));
		}
		else {
			throw new Error ('Invalid route specified: ',route);
		}
	});
}

// Listen for wildcard requests
function routeWildcardRequests() {
	listen('/:entity/:action?/:id?', function(req,res,next) {
		var route = handleWildcardRequest(req,res,next);
		return (processExpressRequest(route.controller,route.action))(req,res,next);
	});
}

// Appropriately route all requests to the specified endpoint
// The implementation of this method depends on the transport type
function listen (url,fn) {
	app.all(url,fn);
}

routeMappedRequests();
routeWildcardRequests();
...
exports.mapSocketRequests = function (app,io) {
	...
	io.sockets.on('connection', function(socket) {
		...
		// Map routes
		socket.on('message', function(request,fn) {
			request = JSON.parse(request);										// Parse request as JSON
			var path = request.url;
			
			// Split url and determine controller/action
			var entityAction = util.parsePath(path);
			// Add controller & action to request data
			request = _.extend(entityAction,request);
			...
			var expressContext = socketInterpreter.interpret(request,fn,socket,app,urlMappings,handleWildcardSocketRequest,processSocketRequest);
			route = route || handleWildcardSocketRequest(request.entity,request.actionName);
