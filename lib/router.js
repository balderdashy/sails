...
// bindRouteFn - the function which will be run as the action
...
function startHandlingWildcardRequests(bindRouteFn) {
	bindRouteFn('/:entity/:action?/:id?', function(req,res,next) {
		// TODO: grab verb from request object
		var verb = null;
...
// Given a not-yet-validated entity and action pair, return the best-guess function.
function generateRoute(requestedEntity,requestedAction) {
	var controller,action,fn;

	// If the entity or action isn't in the routing table, use resourceful routing and Backbone semantics
	// to try and transparently route to the appropriate controller and action.
	...
	// If entity is not specified, build an empty controller on the fly
	controller = (requestedEntity && sails.controllers[requestedEntity]) || {};

	// If the route was found at this point, return working controller+action
	if (_.isFunction(controller[action])) {
		return controller[action];
	}

	// If none of that works, but the proper view exists, return an anonymous action which renders it.
	fs.stat(app.settings.views+"/"+requestedEntity+"/"+action+".ejs", function(err, stat) {
		// If file exists, template and render view
		if (err === null) {
			...
			// If THAT doesn't work, but a model exists for this entity, and the action is a CRUD method 
			// or is a likely plain-english CRUD synonym, use the JSON scaffold controller to return an anonymous action.
			if (qualifiedForScaffold(requestedEntity,requestedAction)) {
				action = crudSynonyms[action] || action;
			}

			// Finally, run any additional middleware (i.e. authentication, request parameter validation, etc.)
			// TODO
		}
	});	
}
...
// 	// If controller exists for this entity
