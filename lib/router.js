	...
	res.view = function(specifiedPath, data, fun) {
		...
		if (_.isArray(data)) { 
			...
			async.forEach(data,function(item,cb){
				res.render(path,item,function(err,renderedView) {
					...
					if (parsedView) {
						...
						err = 'Trying to render a view ('+path+
							') using an array of data, but when rendered '+
							'with the '+_.th(counter)+' data item, the view '+
							'is not a valid JSON (or JSON-like) object:'+
							'\n'+renderedView;
						throw new Error(err);
					}
					cb(err);
				...
				if (err) {
					throw new Error(
						"An error occured while rendering the view ("+path+
		...
		// Otherwise just reender a simple, singular view
			...
			// Stringify data 
			data = _.objMap(data,function(attr) {
				return JSON.stringify(attr);
			});

			// Render into memory first
			res.render(path,data,function(err,renderedView) {
				...
				if (attemptedJSONParse = _u.parseJSONish(renderedView)) {
					...
					// Render the view to the client normally
...
// TODO: create and import route param variable validation
var paramMiddleware = require('./anchor');
...
function processSocketRequest(controllerName, actionName) {
	return function(req, res, app) {
		...
		// Run auth middleware
		authMiddleware.verify(controllerName, actionName, req, res, function() {
