...
function startHandlingMappedRequests(bindRouteFn) {
	// _.shout("urlMappings:",urlMappings,"config",config);
	...
	_.each(urlMappings, function(target, url) {
		...
		bindRouteFn(url,handleMappedRequest(route),route.verb);
	});
}
...
*/
var parseMappedUrl = 
exports.parseMappedUrl = function (url) {
	...
	if(_.isObject(target)) {
		...
		return _.extend({verb: verb},target);
	}
...
/**
 * Handle a mapped request to @target
 */
 var handleMappedRequest = 
 exports.handleMappedRequest = function (target) {
	/**
	 * Handler fn
	 *
...
function startHandlingWildcardRequests(bindRouteFn) {
	...
	bindRouteFn('/:entity/:action?/:id?', handleWildcardRequest(), null);
...
/**
 * Handle a wildcard request
 * NOTE: Target controller/action cannot be determined until request actually comes in
 */
var handleWildcardRequest = 
exports.handleWildcardRequest = function () {
	/**
	 * Handler fn
	 *
	 * @req - the Connect request object
...
/**
 * Given an http request object, with the controller and action already decided,
 * return the proper handler function.
 */
...
function handler(req, res, next) {
	...
	_.shout("HANDLER:",requestedEntity,requestedAction,httpVerb, req.param('controller'));
	...
	res.view = function(specifiedPath, data, fun) {
		...
		if(_.isArray(data)) {
			...
			async.forEach(data, function(item, cb) {
				...
				_.shout("stringifiedItem", stringifiedItem);
				...
				res.render(path, stringifiedItem, function(err, renderedView) {
					...
					var parsedView = _u.parseJSONish(renderedView);
		...
		// Otherwise just reender a simple, singular view
		else {
			...
			res.render(path, stringifiedData, function(err, renderedView) {
				...
				if(attemptedJSONParse = _u.parseJSONish(renderedView)) {
