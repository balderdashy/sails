		...
		async.auto({
			...
			loadAndBuild: function (cb) {
				...
				async.auto([
					...
					userConfig = _.extend(userConfig, mainConfig);

					// Finally, mix in app's local configuration (i.e. `config/local.js`)
					var localConfig = results[1];
					userConfig = self.build(userConfig, _.extend(userConfig, localConfig));
			...
			validate: ['loadAndBuild', function (cb, results) {
				...
				sails.config = deepMerge(sails.config, confAfterBuild);
				...
				sails.config.policies = deepMerge({ "*" : true }, policiesClone);
