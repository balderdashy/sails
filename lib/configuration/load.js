		...
		async.auto({
			...
			loadAndBuild: ['package_json', 'mapOverrides', function (cb, results) {
				...
				async.auto([
					...
					finalConfig = util.merge(finalConfig, overrides);
			...
			validateConfig: ['loadAndBuild', function (cb, results) {
				...
				// var postBuild_Config = userConfig;
				// Intepret session adapter config
				if (util.isObject(userConfig)) {
					if (userConfig.session.adapter === 'memory' && !util.isObject(userConfig.session.store)) {
						userConfig.session.store = new(require('express').session.MemoryStore)();
					}

					if (userConfig.session.adapter === 'redis') {
						userConfig.session.store = new(require('connect-redis')(require('express')))(userConfig.session);
					}

					if (userConfig.session.adapter === 'mongo') {
						userConfig.session.store = new(require('connect-mongo')(require('express')))(userConfig.session);
					}
				}
				...
				// util.merge(sails.config, postBuild_Config);
				...
				// Wipe values out of `sails.config` instead of assigning a new object to it
				...
				// var policiesClone = util.clone(validatedConfig.policies);
				// validatedConfig.policies = util.merge({ "*" : true }, policiesClone);
