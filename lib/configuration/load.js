module.exports = function (sails) {
	...
	return function loadConfig (cb) {
		...
		async.auto({
			...
			/**
			 * Extend `sails` instance w/ data from app's package.json file
			 */
			...
			loadAndBuild: ['package_json', 'mapOverrides', function (cb, results) {
				...
				async.auto([
					...
					var withLocalConfig = _.merge(finalConfig, localConfig);

					// Run the mysterious Configuration.build????!
					// TODO: probably could be removed
					finalConfig = Configuration.build(finalConfig, withLocalConfig);

					// Finally, extend user config with overrides
					finalConfig = _.merge(finalConfig, overrides);
			...
			validateConfig: ['loadAndBuild', function (cb, results) {
				...
				var copyOfDefaults = _.clone(implicitDefaults);
				userConfig = _.merge(copyOfDefaults, userConfig);
				...
				// Extend missing things
				// TODO: move this out if possible
				var postBuild_Config = Configuration.build(implicitDefaults, userConfig);
				
				// Merge that back into sails.config
				// TODO: sails.config at this point is actually just overrides, wtf?!
				// _.merge(sails.config, postBuild_Config);
				...
				var validatedConfig = Configuration.validate(postBuild_Config, copyOfOriginalUserConfig);
				...
				// ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
				// ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
				// TODO: move the rest of this to the very end of the config loading process
				// ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

				// Save final result back into sails.config
				// Use _.extend so that the memory reference to `sails.config`
				// remains valid and refers the the same `sails`
				// (i.e. we can override anything in there, who cares)
				_.extend(sails.config, validatedConfig);

				// Just in case-- debug logging
				console.log('\n\n\n\n\n*********Sails running at ' + process.cwd());
				console.log('\n******\n');
				console.log('sails.config ::', sails.config.appPath);
				console.log('validatedConfig ::', validatedConfig.appPath);
				console.log('userConfig ::', userConfig.appPath);
				console.log('overrides ::', results.mapOverrides.appPath);
				console.log('implicit defaults ::', implicitDefaults.appPath);
				...
				// TODO: move this up-- when ordering is fixed, it shouldnt need to be here
				var policiesClone = _.clone(validatedConfig.policies);
				validatedConfig.policies = _.merge({ "*" : true }, policiesClone);

				// Expose routes as `sails.routes`
				// TODO: move this into `router` module
				sails.routes = validatedConfig.routes || {};
