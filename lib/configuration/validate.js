module.exports = function (sails) {
	...
	return function validateConfiguration (config, originalUserConfig) {
		...
		if (allowedViewEngines.indexOf(config.views.engine) === -1) {
			throw new Error ("Sorry, only the ejs, jade and handlebars view engines are supported at this time.  You specified: " + config.views.engine);
		}
		...
		if (originalUserConfig.rigging) {
			log.error("`rigging` and `asset-rack` have been deprecated in favor of `grunt`");
			log.error("Please check out the sails.js v0.9 migration guide for more information.");
