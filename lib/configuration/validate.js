module.exports = function (sails) {
	...
	return function validateConfiguration (config, originalUserConfig) {
		...
		if ( !config.views.engine.fn ) {
			var dependencyPath = sails.config.appPath + '/node_modules/' + config.views.engine.ext, 
				fn;
			try {

				// fn = require(sails.config.appPath + '/node_modules/consolidate')[config.views.engine.ext];
				// fn = require('sails/consolidate')[config.views.engine.ext];//sails.config.appPath + '/node_modules/consolidate')[config.views.engine.ext];

				// Trick `consolidate` into thinking that it's in another place
				var oldPwd = process.cwd();
				process.chdir(dependencyPath);
				process.chdir('../');

				// Change back to avoid causing quantum disturbance
				process.chdir(oldPwd);

				// Require the view engine and cross the proverbial fingers
				fn = require('../../node_modules/consolidate')[config.views.engine.ext];
