	...
	return function validateConfiguration (configSoFar, originalUserConfig) {
		...
		if (configSoFar.adapters) {
			...
			util.each(configSoFar.adapters, function (legacyAdapterConfig, connectionName) {
				// Ignore `default` (because it's still present for compat.)
				if (connectionName === 'default') {

					var leg_defaultAdapter = legacyAdapterConfig;
					// Expand `default: 'some-adapter-module'` into referenced connection
					// -OR- `{ module: 'sails-disk' }` for backwards compat.
					if ( util.isString(leg_defaultAdapter) ) {
						var referencedConnection = configSoFar.adapters[leg_defaultAdapter];
						if (referencedConnection) {
							configSoFar.adapters.default = referencedConnection;
						}
						else configSoFar.adapters.default = { module: leg_defaultAdapter };
					}
					return;
				}
				...
				// If unspecified, infer adapter/module using the connectionName
				if (!connection.adapter) {
					var possibleModuleName = 'sails-' + connectionName;
					connection.adapter = possibleModuleName;
				}
