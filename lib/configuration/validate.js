module.exports = function (sails) {
	...
	return function validateConfiguration (configSoFar, originalUserConfig) {
		...
		if (configSoFar.adapters) {
			...
			// `config.adapters.default` is being replaced with `config.models.connections`
			if (configSoFar.adapters['default']) {
				sails.log.verbose('Deprecation warning :: Replacing `config.adapters.default` with `config.models.connections`....');
				configSoFar.models.connections = configSoFar.adapters['default'];
			}
			...
			// var defaultAdapter = configSoFar.adapters['default'];
			// if (defaultAdapter && util.isString(defaultAdapter)) {
			// 	configSoFar.adapters['default'] = configSoFar.adapters[defaultAdapter];
			// }
