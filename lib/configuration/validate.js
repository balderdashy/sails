	...
	return function validateConfiguration (configSoFar, originalUserConfig) {
		...
		if ( !configSoFar.views.engine.fn ) {
			...
			try {
				...
				sails.log.error('Your configured server-side view engine (' + configSoFar.views.engine.ext + ') could not be found.');
				sails.log.error('Usually, this just means you need to install a dependency.');
				sails.log.error('To install ' + configSoFar.views.engine.ext + ', run:  `npm install ' + configSoFar.views.engine.ext + ' --save`');
