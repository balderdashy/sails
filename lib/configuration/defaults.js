module.exports = function (sails) {
	...
	return function defaultConfig (userConfig) {
		...
		_.extend(paths, {
			...
			blueprints		: paths.app + '/api/blueprints',
		...
		return {
			...
			404: function notFound (err, req, res) {
				res.send(err || undefined, 404);
			},

			// Default 500 (server error) handler
			500: function (err, req, res) {
				res.send(err || undefined, 500);
			},

			// Default 403 (forbidden) handler
			403: function (err, req, res) {
				res.send(err || undefined, 403);
			},

			// Default 400 (bad request) handler
			400: function (err, previous, req, res) {
				res.send(err || undefined, 400);
			},

			blueprints: ['actions', 'shortcuts', 'rest'],
