...
module.exports = function (defaults,userConfig) {
	...
	if (_.isObject(result.session)) {	
		...
		if (result.session.adapter === 'mongo') {
			sails.log.error('Built-in Mongo session support disabled in 0.8.x to prevent installation issues.  If you need help configuring your sessions to use Mongo, please reach out to the Sails Google Group or #sailsjs on freenode.');
			process.exit(1);
