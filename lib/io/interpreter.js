...
exports.route = function(socketReq, fn, socket) {
	...
	console.log('session store: ', sails.config.session.store);
	sails.config.session.store.get(sessionKey, function (err, sessionData) {

		// sails.log.error('Error retrieving session:');
		// sails.log.error(err);
		// sails.log.verbose(sessionData);
		// sails.log.verbose('STORE');
		...
		console.log('it should be there!!!',expressContext.req.session);
