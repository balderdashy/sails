...
exports.definition = function(modelName) {
	...
	var actions = {
		...
		// 	// Serve the index view if it exists
		// 	if(res.viewExists) return res.view();
			
		// 	// Otherwise serve find() or findAll()
		// 	else {
		// 		if(req.param('id')) return actions.find(req, res);
		// 		else return actions.findAll(req, res);
		// 	}
		// },
		...
		find: function(req, res) {
			...
			Model.find(req.param('id')).done(function(err, model) {
				...
				//and pass down model as "model"
				else if (res.viewExists && !(req.isAjax || req.isSocket || req.isJson)) return res.view({
					model: model.values
				});
		...
		findAll: function(req, res) {
			...
			finding.done(function afterFound(err, models) {
				...
				var modelValues = util.objMap(models, function (model) {
					return model.values;
				});
