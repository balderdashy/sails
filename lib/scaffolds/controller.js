exports.definition = function(modelName) {
	...
	var actions = {
		...
		// Default action, if /controller is hit
		index: function(req, res) {

			// If this is an AJAX or socket request, or seems JSON-ish
			// or if the view doesn't exist,
			// serve the scaffolded API
			if(req.isAjax || req.isSocket || req.isJson || !res.viewExists) {
				if(req.param('id')) actions.find(req, res);
				else actions.findAll(req, res);
			} 

			// Otherwise serve the view
		...
		find: function(req, res) {
			...
			Model.find(req.param('id')).done(function(err, model) {
				...
				// Use the view if one exists, and pass down model as "model"
				else if (res.viewExists) return res.view({
					model: model
				});
		...
		findAll: function(req, res) {
			...
			finding.done(function afterFound(err, models) {
				...
				// Use the view if one exists, and pass down model as "model"
				if (res.viewExists) return res.view({
					models: models
				});

				// Otherwise serve a JSON API
				else return res.json(models);
			});
		},
