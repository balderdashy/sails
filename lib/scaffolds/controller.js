exports.definition = function(modelName) {
	...
	return {
		...
		findAll: function(req, res) {
			...
			// If "filter" is explicitly enabled, filter the parameters
			// (really useful for when a client's view-model is bundled in the same object as its api model)
			// (e.g. Backbone.js)
			var where = req.param("filter") ? Model.filter(query) : query;
		...
		create: function(req, res) {
			...
			// (really useful for when a client's view-model is bundled in the same object as its api model)
			// (e.g. Backbone.js)
			params = req.param("filter") ? Model.filter(params) : params;
