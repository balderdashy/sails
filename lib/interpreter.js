...
exports.route = function(socketReq,fn,socket) {
	...
	if(entityAction) {
		...
		handlerFn = router.handleMappedRequest(entityAction);
	} else {
		handlerFn = router.handleWildcardRequest();
	}
...
exports.interpret = function(socketIOData, socketIOCallback, socket) {//, app, urlMappings, handleWildcardSocketRequest, processSocketRequest) {
	...
	var res = exports.res = {
		...
		render: function(view, options, fn) {
			_.shout("View",view,"ext:",_.fileExtension(view));
			// If no file extension is specified, use the default for the rendering engine
			...
			fs.readFile(app.settings.views + "/" + view, "utf-8", function(err, template) {
				if(err) {
					...
					debug.warn(err);
				} else res.send(ejs.render(template, _.extend({
					session: req.session,
