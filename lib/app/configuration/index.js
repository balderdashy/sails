  ...
  function Configuration() {
    ...
    this.defaults = function defaultConfig(appPath) {
      ...
      return {
        ...
        hooks: _.reduce(DEFAULT_HOOKS, function (memo, hookName, hookDef) {

          // if `true`, the hook is bundled in the Sails core module
          // (look in the `lib/hooks` folder in core)
          if (hookDef === true) {
            memo[hookName] = require('../../hooks/'+hookName);
          }
          else {
            memo[hookName] = hookDef;
          }
          return memo;
        }, {}) || {},
