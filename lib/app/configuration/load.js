    ...
    async.auto({
        ...
        mapOverrides: function(cb) {
          ...
          overrides = _.merge(overrides, {
            ...
            // `--prod`, `--staging` and `--dev` command-line argument
            environment: overrides.prod ? 'production' : (overrides.staging ? 'staging' : (overrides.dev ? 'development' : undefined))
        ...
        mixinDefaults: ['mapOverrides',
          function(cb, results) {
            ...
            if (mergedConfig.environment && !process.env['NODE_ENV']) {
