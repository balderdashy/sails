...
module.exports = function exposeGlobals() {
  ...
  // If globals config is provided, it must be either `false` or a dictionary.
    ...
    throw flaverr('E_BAD_GLOBAL_CONFIG', new Error('In Sails v1.0, if `sails.config.globals` is defined, it must either be a dictionary or `false`.  If `undefined`, it will default to `false` (no globals).'));
  }
