    ...
    async.series(
      {
        ...
        validate: function(cb) {
          if (hooks.controllers) {
            sails.log.error('=================================================================================');
            sails.log.error('Error loading `controllers` hook:');
            sails.log.error('As of Sails v1, `controllers` can no longer be disabled/enabled as hooks.');
            sails.log.error('Instead, Sails core now understands controller actions as first-class citizens.');
            sails.log.error('See the Sails v1.0 upgrade guide: http://sailsjs.com/upgrading');
            sails.log.error('=================================================================================');
            return cb(new Error('Invalid hook'));
          }
          return cb();
        },
