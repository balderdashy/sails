...
module.exports = function(sails) {
  ...
  return function initializeHooks(hooks, cb) {
    ...
    function loadHook(id, cb) {
      ...
      var timeoutInterval = (sails.config[hooks[id].configKey || id] && sails.config[hooks[id].configKey || id]._hookTimeout) || 10000;
      var hookTimeout;
      if (id != 'userhooks') {
    ...
    async.series({
        ...
        defaults: function(cb) {
          async.each(_.without(_.keys(hooks), 'userconfig', 'moduleloader'), function (id, cb) {
            var hook = hooks[id];
            applyDefaults(hook);
            // Defer to next tick to allow other stuff to happen
            process.nextTick(cb);
