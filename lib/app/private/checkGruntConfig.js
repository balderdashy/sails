...
module.exports = function(sails) {
  ...
  return function(results, cb) {
    ...
    var appPackageJSON;
    try {
      appPackageJSON = JSON.parse(Filesystem.readSync({source: path.resolve(process.cwd(), 'package.json')}).execSync());
    } catch (e) {
    ...
    var hash = (function() {
      try {
        var Gruntfile = Filesystem.readSync({source: path.resolve(process.cwd(), 'Gruntfile.js')}).execSync();
    ...
    if (_.contains(GRUNT_FILE_HASHES, hash)) {
      ...
      sails.log.debug('Run `npm install --save sails-hook-grunt` to continue using Grunt with your app.');
      sails.log.debug();
    }
