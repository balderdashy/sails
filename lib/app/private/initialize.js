...
module.exports = function initialize(cb) {
  ...
  sails.runBootstrap(function afterBootstrap(err) {
    ...
    if (!util.isObject(sails.hooks)) { return cb(new Error('Consistency violation: `sails.hooks` should be a dictionary.')); }
    async.each(Object.keys(sails.hooks), function (hookName, next){
      if (!util.isFunction(sails.hooks[hookName].handleLift)) {
        return next();
      }
