module.exports = function (sails) {
	...
	function loadHookDefinitions (hooks, cb) {
		...
		// Mix in an `identity` property to each hook
		util.each(hooks, function ( hook, key ) {
			hook.identity = key.toLowerCase();
		});

		return cb();


		// TODO: deprecate auto-loading
		// (hooks must be included as part of `configOverride`)
	}
