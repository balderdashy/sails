	...
	this.lift = function (configOverride, cb) {
		...
		async.series([
			...
			function (cb) {
				...
				// Provide a heartbeat to let user know that Sails is working
				// only relevant on very slow systems or in apps with very complex
				// bootstrap functions
				util.defer(function maybeLoggerIsReady () {
					sails.log && sails.log.info('Configuring app...');
				}, 1);
			},
