	...
	this.lower = function (cb) {
		...
		beforeShutdown(function (err) {
			...
			util.each(sails.childProcesses, function kill (childProcess) {
				sails.log('Killing grunt child process...');
				childProcess.kill('SIGHUP');
			});

			// Shut down HTTP server
			// TODO: defer this to the http hook-- use sails.emit('lowering')
			// TODO: Shut down Socket server (right now it's automatic)
			// TODO: wait for all attached servers to stop
			sails.express.server.close(function (err) {
				if (err) sails.log.error(err);
				cb();
			});
		});
