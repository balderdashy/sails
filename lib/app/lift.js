...
function _printSuccessMsg(sails) {
  ...
  if (!(sails.config.log && sails.config.log.noShip)) {
    ...
    if (sails.plugins.list().length > 0) _printLoaderMsg("Plugins loaded :", sails.plugins.list());
    if (sails.extensions.list().length > 0) _printLoaderMsg("Extensions loaded :", sails.extensions.list());
    sails.log('--------------------------------------------------------'.grey);
