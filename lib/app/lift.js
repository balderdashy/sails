...
var _printPluginList = function(sails){
  sails.log.blank();
  sails.log('Plugins loaded :');
  _.forEach(sails.plugins.list(), function(plugin){
    sails.log("\t " + plugin + " " + figures.tick);
  });
};

var _pringExtensionList = function(sails){
  sails.log.blank();
  sails.log('Extensions loaded :');
  _.forEach(sails.config.extensions, function(plugin){
    sails.log("\t " + plugin + " " + figures.tick);
  });
};
...
function _printSuccessMsg(sails) {
  ...
  if (!(sails.config.log && sails.config.log.noShip)) {
    ...
    if (sails.plugins.list().length > 0) _printPluginList(sails);
    sails.log('--------------------------------------------------------'.grey);
