...
exports.route = function(socketReq, fn, socket) {
	...
	loadSession(socket, function (err, sessionData) {
		...
		expressContext.req.session = sessionData || new Error('Session not available.');
		...
		handlerFn(expressContext.req, expressContext.res, function () {
			...
			// console.log('handlerFn callback from socket request was triggered!');

			// // Save session
			// saveSession(socket, expressContext.req.session, function (err) {
			// 	if (err) {
			// 		sails.log.error('Error saving session: ');
			// 		sails.log.error(err);
			// 		return expressContext.res.send(err, 500);
			// 	}
			// });
		});
	});
...
function loadSession (socket, cb) {
	...
	sails.config.session.store.get(sessionKey, cb);
}

// // Save new session data to the session store
// function saveSession (sessionKey, newSessionData, cb) {
// 	// var sessionKey = 'sess:'+socket.handshake.sessionID;
// 	var sessionKey = socket.handshake.sessionID;
// 	var newSessionData = socket.
// 	sails.config.session.store.set(sessionKey, newSessionData, cb);
// }
...
exports.interpret = function(socketIOData, socketIOCallback, socket) {
	...
	var req = exports.req = {
		...
		// TODO: grab actual protocol from socket.io transport method (to respect wss://)
		protocol: 'ws',
	...
	var res = exports.res = _.extend(new ResStream(), {
		...
		send: function(body, status) {
			...
			sails.config.session.store.set(sessionKey, req.session, function (err) {
				if (err) {
