...
exports.route = function(socketReq, fn, socket) {
	...
	loadSession(socket, function (err, sessionData) {
		if (err) return expressContext.res.send(err, 500);

		// Provide access to session data as req.session
		expressContext.req.session = sessionData;

		// Call handler action using the simulated express context
		handlerFn(expressContext.req, expressContext.res, function (a,b,c) {
...
function saveSession (socket, newSessionData, cb) {
	var sessionKey = 'sess:'+socket.handshake.sessionId;
	console.log("SESSION:",newSessionData);
