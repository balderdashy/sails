...
exports.route = function(socketReq, fn, socket) {
	...
	var url = socketReq.url.substr(0, socketReq.url.indexOf('?'));
	var entityAction = Router.fetchRoute(url, getVerb(socketReq));
...
exports.interpret = function(socketIOData, socketIOCallback, socket) {
	...
	if (socketIOData.url.indexOf('?') !== -1) {
	  req.query = Qs.parse(socketIOData.url.substr(socketIOData.url.indexOf('?') + 1));
	}
