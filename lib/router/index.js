...
function Router(options) {
  ...
  // Return the array of sorted route addresses;
...
Router.prototype.flush = function(routes) {
  ...
  sortedRouteAddresses = sortRoutes.apply(this, [this.explicitRoutes]);
...
function sortRoutes (_routes) {

  var sails = this.sails;

  // Make a shallow clone of the routes
  var routes = _.clone(_routes);

  // First, find the # of parts in the longest route address
  var maxParts = _.reduce(_.keys(routes), function(memo, address) {
    ...
    if (parts.length > memo) {memo = parts.length;}
  ...
  var rankedAddresses = _.reduce(_.keys(routes), function(memo, address) {
