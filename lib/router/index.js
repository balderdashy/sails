module.exports = function(sails) {
	...
	function Router (options) {
		...
		Router.prototype.route = function(req, res, cb) {
			...
			this._expressApp.router(req, res, this.wildcard(req, res));
		...
		Router.prototype.unbind = function(route, index) {
			...
			var newRoutes = [];
			_.each(this._expressApp.routes[route.method], function(expressRoute) {
				if (expressRoute.path != route.path) {
					newRoutes.push(expressRoute);
				}
			});
			this._expressApp.routes[route.method] = newRoutes;
		...
		Router.prototype.reset = function(verb) {
			...
			if (!verb) {
				_.each(this._expressApp.routes, function(routes, method) {
					this.reset(method);
				}, this);
			...
			var routesToUnbind = this._expressApp.routes[verb] || [];
		...
		Router.prototype.flush = function() {
			...
			// but BEFORE implicit `after` routes
			// 
			// TODO:
			// Is this necessary?
			// Remove if it isn't-- it adds complexity.
