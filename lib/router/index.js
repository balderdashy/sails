...
function handleRequest(target) {
	...
	return function(req, res, next) {
		...
		var myTarget = {
			controller: (target && target.controller) || guessController(req),
...
/**
 * Mostly the controller will be the one requested in the params. However, we
 * also want to allow pluralized controllers, so for a model 'User' the 
 * /users route will be active, if configured as such.
 */
function guessController(req) {
  if (sails.config.pluralizeController === true) {
    return sails.util.singularizeString(req.param('entity'));
  } else {
