...
function Router ( options ) {
	...
	// Build empty middleware object and populate it with policies
	this.middleware = {
		_policies: _.clone(sails.policies)
	};
 ...
 * @param {String|Object|Array|Function} middleware
 * @param {String} verb
 * @api private
 */
...
Router.prototype.bind = function ( path, middleware, verb ) {
	...
	// Handle middleware chain syntax
	...
	// Inline middleware function
		...
		this.app[verb || 'all'](path, middleware);
	}

	else {
		this.log.error('Ignored attempt to bind route to invalid middleware:');
		this.log.error(middleware);
	}
};
...
/**
 * Bind, in order of descending preference:
 *
 * -> static routes (routes config)
 * -> resourceful/view routes (config + middleware table)
 * -> blueprint routes (blueprint config + middleware table)
 * -> default 404 handler
 * -> default 500 handler
 *
 * @api private
...
Router.prototype.route = function ( ) {
	...
	// Replicate each route from the config
	_.each(this.staticRoutes, function (target, path) {

		// Interpret target and lookup appropriate middleware
...
/**
 * (Re)build middleware cache from controllers, policies, config, views, and models.
 *
 * @api private
 */
...
Router.prototype.build = function ( ) {
	...
	if (sails.controllers._policies) {
		sails.log.error('_policies is a reserved identity.  You cannot name your controller that!');
		throw new Error();
	}


	_.each(sails.controllers, function (controller, identity) {

		var chain = this.middleware[identity] = {};

		// If this controller qualifies for blueprints, mix them in
		// todo

		// If there are any views which correspond with unspecified actions, create middleware to serve them
		// todo
		
		// Copy over controller's methods
		 _.extend(chain, _.clone(controller));
