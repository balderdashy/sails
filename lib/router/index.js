module.exports = function(sails) {
	...
	function Router(options) {
		...
		/**
		 * Check if `route` matches `path`, if so
		 * populate `.params`.
		 *
		 * This function should be equivalent, if not identical, to the 
		 * route matching logic in the Express core.
		 *
		 * @param {Object} route
		 * @param {String} path
		 * @return {Boolean}
		...
		Router.prototype.unbind = function(route, index) {
			...
			// Remove reference to route
			this.routes[verb].splice(index, 1);
		};
		...
		Router.prototype.reset = function(verb) {
			...
			_.each(this.routes[verb] || [], this.unbind, this);
