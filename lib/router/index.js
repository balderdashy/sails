 ...
 * @param  {Object|String} target The route target to get an action identity for
...
Router.prototype.getActionIdentityForTarget = function(target) {
  ...
  if (_.isObject(target)) {
    ...
    if (target.controller) {
      if (!target.action) {
        throw new Error('If `controller` is specified, `action` must be also!');
      }
    ...
    // Attempt to handle `{action: 'user.create'}` target.
      ...
      throw new Error('If target is an object, it must contain an `action` property!');
    }
  }
  ...
  // 'user/create'
    ...
    // Normalize the action identity by replacing `Controller.` with `/`
    actionIdentity = target.replace(/Controller\./,'/');

  }

  else {
    throw new Error('Target must be an object or string!');
  }
