...
Router.prototype.getActionIdentityForTarget = function(target) {
  ...
  if (_.isObject(target) && !_.isArray(target) && !_.isFunction(target)) {
    ...
    // Bail if the action
    if (actionIdentity.match(/^\w+Controller/) || !actionIdentity.match(/^\w+[a-zA-Z\/-]*$/)) {
      var didYouMean = '';
      if (!actionIdentity.match(/[^a-zA-Z.\/-]/)) {
  ...
  // 'user/create'
    ...
    if (actionIdentity.match(/Controller\//) || !actionIdentity.match(/^\w+[a-zA-Z\/-]*$/)) {
      throw flaverr('E_NOT_ACTION_TARGET', new Error(
        '\nCould not parse invalid action `' + actionIdentity + '`.\n'+
        'See http://sailsjs.com/docs/concepts/routes/custom-routes#?controller-action-target-syntax\n'+
        'for more info on controller/action and standalone action route syntax.\n'
      ));
    }
