...
function Router(options) {
  ...
  // TODO: instead, use: https://www.npmjs.org/package/path-to-regexp
...
Router.prototype.load = function(cb) {
  ...
  if (sails.hooks.session) {
    sails.after('hook:session:loaded', function (){
      sails._privateCpMware = cookieParser(sails.config.session.secret);
      sails._privateSessionMiddleware = session(sails.config.session);
    });
  }
...
Router.prototype.reset = function() {
  ...
  _.each(this._privateRouter.stack, function(stack) {
    if (!_.isObject(stack.route)) return;
    var route = {
  ...
  if(this._privateRouter.stack.length) console.log(this._privateRouter.stack)
