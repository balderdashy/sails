...
Router.prototype.route = function(req, res) {
  ...
  console.log('\n\n\n\n=======================\nReceived request to %s %s',req.method,req.url);
...
function parseCookies (req, res, next){
  ...
  // If no cookie exists, generate a random one
  // (this will create a new session w/ every request)
  // if (!req.headers.cookie) {
  //   try {
  //     req.headers.cookie = req._sails.config.session.key + '=' + uuid.v1();
  //   }
  //   catch (e){
  //     req._sails.log.verbose('Failed to generate new cookie.  Error:',e);
  //   }
  // }
...
function loadSession (req, res, next){
  ...
  if (req._sails.config.session && req._sails.config.session.store) {
    req._sails.log.verbose('Running session middleware on interpreted virtual request with cookie:', util.inspect(req.headers.cookie, false, null));
    // console.log('Running session middleware on interpreted virtual request with cookie:', req.headers.cookie);
    // console.log('Session config:',req._sails.config.session);

    // TODO: this line of code does not need to be run for every request
    // (i.e. store preconfigured session middleware as a private property on i.e. the app instance)
    var sessionMiddleware = Express.session(req._sails.config.session);
