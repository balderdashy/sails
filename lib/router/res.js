...
function ToSaveSession(app) {
   ...
   * @param  {Function} cb
   */
  return function saveSession(session, cb) {

    // If we have a session on the request, save it
    // Otherwise, process the callback on the next tick
    (function selfInvokingFn(next){
      if (session) {
        session.save(next);
      } else {
        process.nextTick(next);
      }
    })(function eitherWay(err) {
      if (err) {
        app.log.error('Session could not be persisted. Details:', err);
      }
      cb();
    });
  };
