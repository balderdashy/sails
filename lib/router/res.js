...
module.exports = function buildResponse (req, _res) {
  ...
  if (res._clientCallback) {
    ...
    // Build `res._clientRes` if one was not provided
    if (!res._clientRes) {
    ...
    res._clientRes.on('finish', function() {
      ...
      res._clientRes.body = Buffer.concat(res._clientRes._readableState.buffer).toString();
      try {
        res._clientRes.body = JSON.parse(res._clientRes.body);
      } catch (e) {}
  ...
  res.send = res.send || function send_shim () {
    ...
    // (this has to happen in `send()` because the code/headers might have changed)
    if (res._clientRes) {
