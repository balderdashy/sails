...
function bindAction(path, target, verb, options) {
  ...
  if (_.isObject(target)) {
    ...
    if (target.controller) {
      ...
      actionIdentity = target.controller.replace('Controller', '').toLowerCase() + '/' + target.action.toLowerCase();
    }
  ...
  // 'user/create'
    ...
    // Normalize the action identity by replacing `Controller.` and any `.` chars
    // with `/`, and lower-casing.
    actionIdentity = target.replace(/Controller\./,'/').replace(/\./g, '/').toLowerCase();
