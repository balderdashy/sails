module.exports = function (sails) {
	...
	function bind ( path, target, verb ) {
		...
		if (util.isObject(target) && !util.isFunction(target) && !util.isArray(target)) {
			...
			if (!util.isUndefined(target.middleware)) {
	...
	function bindController ( path, target, verb ) {
		...
		var controllerId = _normalizeControllerId(target.controller);
	...
	function bindString ( path, target, verb ) {
		...
		// If target matches a controller (or view) in the middleware registry
		// go ahead and assume that this is a dot notation route
		// TODO: pull this logic into the controllers/views hooks
		if (parsedTarget[1] && (
			sails.middleware.controllers[parsedTarget[1]] ||
			sails.middleware.views[parsedTarget[1]]
			)
		) {
	...
	/**
	 * Accept either `FooController` or `foo`, then lower-case it
	 */

	function _normalizeControllerId (controllerId) {
