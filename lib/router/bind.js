module.exports = function (sails) {
	...
	var resourceArr = [
		{"method" : "get", "action" : "index", "urlPath" : "" },
		{"method" : "get", "action" : "add", "urlPath" : "/add" },
		{"method" : "get", "action" : "show", "urlPath" : "/:id" },
		{"method" : "get", "action" : "edit", "urlPath" : "/:id/edit" },
		{"method" : "post", "action" : "create", "urlPath" : "" },
		{"method" : "put", "action" : "update", "urlPath" : "/:id" },
		{"method" : "delete", "action" : "destroy", "urlPath" : "/:id" }
	];
	...
	function bind ( path, target, verb ) {
		...
		if (util.isObject(target) && !util.isFunction(target) && !util.isArray(target)) {
			...
			if ( !verb ) {
				if ( target.resource ) {
					// Support RAILS type REST based url mapping with methods like show, create, destroy etc
					// eg. [ "/user" : { resource : "FooController" } ]
					// will lay down all the rest based urls mapped to actions in FooController
					sails.log.verbose("Generating resourceful routes for \"" + path + "\"");
					return resourceArr.forEach(function(route){
						var routePath = path + route.urlPath;
						bindString(routePath, target.resource + "." + route.action, route.method);
					});
				} else {
					// Support custom resourceful shortcut mappings
					// e.g. { post: 'FooController.bar', get: '...', /* ... */ }
