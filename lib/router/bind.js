module.exports = function (sails) {
	...
	function bind ( path, target, verb ) {
		...
		console.log("WHEN DOES IT BREAK ?!?!?!?!?", sails.middleware.controllers, target);
	...
	function bindController ( path, target, verb ) {
		...
		sails.log(' *** * > Binding controller thing::   targetFn :: ',targetFn);
		sails.log('************');

		// Make sure the controller is usable
		if (_.isArray(targetFn) {

		}

		if (!_.isFunction(targetFn)) {
			sails.log('PROBLEM: targetFn is ', { test: function yes () {} } );
			return process.exit(1);
		}

		// Bind intercepted middleware function to route
		_bind(path, wrapperFn, verb);


		// Wrap up the controller middleware to supply access to
		// the original target when requests comes in
		function wrapperFn (req, res, next) {
