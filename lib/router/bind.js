...
function bindFunction(path, fn, verb, options) {
  ...
  fn._middlewareType = fn._middlewareType || options._middlewareType || (fn.name && ('FUNCTION: ' + fn.name));
  sails.log.silly('Binding route :: ', verb || '', path, fn._middlewareType?('('+fn._middlewareType+')'):'');
  ...
  sails.router._privateRouter[targetVerb](path, targetFn);
