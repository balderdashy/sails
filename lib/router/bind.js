...
function bindAction(path, target, verb, options) {
  ...
  if (_.isObject(target)) {
    ...
    if (target.controller) {
      ...
      if (target.controller.indexOf('/') > -1) {
        sails.log.warn('Using slashes in route target syntax is deprecated. '+
                       'Binding `' + target.controller + '.' + target.action + '` to `' + path + '` for now, but consider changing this to { action: \'' + actionIdentity + '\' }.');
      }
  ...
  // 'user.create'
    ...
    if (target.indexOf('/') > -1) {
      sails.log.warn('Using slashes in route target syntax is deprecated. '+
                     'Binding `' + target + '` to `' + path + '` for now, but consider changing this to { action: \'' + actionIdentity + '\' }.');
    }
