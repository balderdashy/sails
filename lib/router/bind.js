	...
	function bindController ( path, target, verb ) {
		...
		function controllerHandler (originalFn) {
			...
			if ( !_.isFunction(originalFn) ) {
				sails.log.error(target.controller + '.' + target.action + ' :: Ignoring invalid attempt to bind route to a non-function controller:', originalFn, 'for path: ', path, 'and verb: ', verb);
				return;
			}
