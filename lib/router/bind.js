module.exports = function (sails) {
	...
	function bindView ( path, target, verb ) {
		...
		// If `locals` were specified, use them
		if ( ! util.isPlainObject(target.locals) ) {
			target.locals = {};
		}
		...
		if ( util.isPlainObject(viewMiddleware) ) {
			viewMiddleware = function applyRouteLocals (req,res,next) {
				util.extend(res.locals, target.locals);
				return viewMiddleware[subview](req,res,next);
			};
		}
