...
module.exports = function (sails) {
	...
	function bindArray ( path, target, verb, options ) {
		...
		// Bind a closure that calls all of the middleware functions in order,
		// then this will be bound to the internal router instead of binding
		// each individual function directly.
		// 
		// This is here mainly to allow users to change route params (`req.params`)
		// in a policy and have the change reflect in subsequent policies/stuff.
		// May also have the effect of an optimization.
		bind(path, function compoundFn(req, res, next) {

			// Call each policy/handler in order
			// (except the last one)
			var lastFn = target.pop();

			async.eachSeries(target, function actuallyCallEachFn (fn, cb) {
				fn(req, res, cb);
			}, function runLastFn(err) {
