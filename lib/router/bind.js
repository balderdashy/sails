...
function bindFunction(path, fn, verb, options) {
  ...
  var enhancedFn = function routeTargetFnWrapper(req, res, next) {
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // TODO: Before invoking or emitting w/ `fn` check to see if it's an async function, and if so, attach
    // `.catch()` to its return value (which will be a promise) in order to handle rejections in the same way
    // we handle exceptions that are thrown synchronously (mainly for the purpose of being able to use async/await)
