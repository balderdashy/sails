module.exports = function (sails) {
	...
	function _bind ( path, target, verb ) {
		...
		// If target is an error middleware function, file it away accordingly
		// TODO
		...
		if (!verb) {
			_.each(sails.router.routes, function (subRoutes, verb) {
				subRoutes.push(_.clone(route));
			}, this);
		}
		// Otherwise save the route to the particular verb specified
		else sails.router.routes[verb].push(route);


		// Emit an event to make hooks aware that a route was bound
		sails.emit('router:bind', route);
	...
	function attachAuxRoutingMiddleware ( path, target, verb ) {
		...
		_bind (path, function (req, res, next) {
			...
			if (!target) {
				target = util.parsePath(req.path);
			}
