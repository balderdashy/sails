module.exports = function (sails) {
	...
	function bind ( path, target, verb ) {
		...
		if (util.isPlainObject(target)) {
			...
			// Support resourceful shortcut mappings
			// e.g. { post: 'FooController.bar', get: '...', /* ... */ }
			if ( !verb ) {
				var foundVerb = false;
				util.each(['get', 'post', 'put', 'delete'], function (verb) {
					if (target[verb]) {
						bind(path, target[verb], verb);
						foundVerb = true;
					}
				});
				if (foundVerb) {
		...
		if (util.isString(target) && target.match(/^(http:|\/)/)) {
	...
	function bindRedirect ( path, target, verb ) {
		...
		return _bind(path, function (req, res) {
