module.exports = function(sails) {
	...
	return function socketAttemptingToConnect (handshake, accept) {
		...
		// If metadata about the API version of the connecting client was not
		// provided in the version string, assume version = 0.9
		// (prior to 0.10, there wasn't any API versioning)
		if (!handshake.query[API_META_PARAMS.version]) {
			handshake.query[API_META_PARAMS.version] = '0.9.0';
		}

		var sdk = {
			version: handshake.query[API_META_PARAMS.version],
			platform: handshake.query[API_META_PARAMS.platform],
			language: handshake.query[API_META_PARAMS.language]
		};
