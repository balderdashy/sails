...
module.exports = function (sails) {
  ...
  return function interpretSocketReq (socketReq, socketIOCallback, socket, messageName, cb) {
    ...
    function sessionReady (err, session) {
    ...
    if(sails.config.hooks.session) {
      // Populate req.session using shared session store
      sails.session.fromSocket(req.socket, sessionReady);
    }
    else {
      // We don't need no sessions!
      sessionReady(null, null);
    }
