  ...
  return {
    ...
    defaults: {
      ...
      sockets: {
        ...
        // Whether to enable the firehose socket. This is designed for development use only.
        enableFirehose: true
    ...
    routes: {
      ...
      before: {
        ...
        'get /firehose': function(req, res) {
          if(!sails.config.sockets.enableFirehose ) {
            res.send(404);
          }
          if (!req.isSocket) {
          ...
          if (process.env.NODE_ENV !== 'development') {
            sails.log.warn('A client socket was just subscribed to the firehose.  But the Sails firehose is '+
              'designed for development, not production!  You are probably seeing this message because new Sails '+
              'apps come with a client-side script (`assets/js/connection.example.js`) that subscribes to the '+
              'firehose by default. If you aren\'t actually using the firehose, and just want to prevent this message ' +
              'from displaying, remove the relevant lines from that file (i.e. `socket.get("firehose", ...)`)');
          }
