module.exports = function (sails) {
	...
	return function interpret (socketReq, socketIOCallback, socket, verb, cb) {
		...
		var res = _.extend(new ResStream(), {
			...
			// Simulation of HTTP status code
			// see also: http://expressjs.com/api.html#res.status
			statusCode: 200,

			// Chainable, just like Express's res.status(code)
			// see also: http://expressjs.com/api.html#res.status
			...
			send:
			saveSessionAndThen(
			function sendResponse (	/* body [,statusCode] */
									/* statusCode, body */ ) {

				// Support (body,statusCode) or (statusCode, body)
				var body, statusCode;
				...
				// Apply statusCode if specified, or use the one already set
				this.statusCode = statusCode || this.statusCode;


				if ( !_.isFunction(socketIOCallback) ) {
					sails.log.error('Could not send response over socket-- invalid callback :: ',socketIOCallback);
				}
			...
			// Send json response
			json: function sendJSONResponse(	/* obj [,statusCode] */
												/* statusCode, obj */ ) {

				// Support (obj,statusCode) or (statusCode, obj)
				var obj, statusCode;
				if (2 === arguments.length) {

					// res.json(obj, status) backwards compat.
					if ('number' !== typeof obj && 'number' === typeof arguments[1]) {
						statusCode = arguments[1];
						obj = arguments[0];
					}
					else {
				...
				try {
					...
					return this.send('res.json() :: Error stringifying JSON!\n' + e, 500);
				}
			...
			jsonp: function (obj, statusCode) {
				// Chainable
				return this.json(obj, statusCode);
			},
		...
		// Locals object for use in rendering views
		function Locals (){
		...
		// Chainable, set the value of a view local
