module.exports = function(sails) {
	...
	return {
        ...
        sails.router.bind('/*', function(req, res, next) {
          // If we have an origin header, and we set an access-control-allow-origin previously,
          // and the two don't match, then we don't approve of this origin and shouldn't continue
          // with this request.
          if (req.headers.origin && !_.isUndefined(res.get('access-control-allow-origin')) && res.get('access-control-allow-origin') != req.headers.origin) {
            return res.forbidden();
          }
          return next();
        }, null, {_middlewareType: 'CORS catchall'});
