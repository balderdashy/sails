  ...
  return {
    ...
    configure: function() {
      ...
      if (sails.config.cors.origin === '*' && sails.config.cors.credentials === true && sails.config.cors.allowAnyOriginWithCredentialsUnsafe !== true) {
        sails.log.error('Invalid CORS settings: if `origin` is \'*\', `credentials` cannot be `true` unless `allowAnyOriginWithCredentialsUnsafe` is also true (setting `credentials` to `false` for you).');
      ...
      if (_.isString(sails.config.cors.origin) && sails.config.cors.origin !== '*') {
        sails.config.cors.origin = sails.config.cors.origin.split(',');
      }
    ...
    initialize: function(cb) {
      ...
      sails.on('router:before', function () {
        ...
        if (sails.config.cors.allRoutes === true) {
          sails.router.bind('/*', cors(globalCorsModuleSettings), 'all', {_middlewareType: 'CORS HOOK: sendHeaders'});
        }
        // Otherwise, default to blocking all cross-origin requests.
        else {
          sails.router.bind('/*', cors({origin: false}), null, {_middlewareType: 'CORS HOOK: clearHeaders'});
        }
          ...
          if (!_.isUndefined(config.cors)) {
            ...
            if (config.cors === true) {
              if (!sails.config.cors.allRoutes) {
                ...
                sails.router.bind(route, cors(globalCorsModuleSettings), null, {_middlewareType: 'CORS HOOK: setHeaders'});
              }
            }
            ...
            // If cors is set to `false`, clear the CORS headers for this route
              ...
              sails.router.bind(route, cors({origin: false}), 'all', {_middlewareType: 'CORS HOOK: clearHeaders'});
              return;
            }
            ...
            // Else if cors is set to a string, use that has the origin
              ...
              sails.router.bind(route, cors(_.extend({}, globalCorsModuleSettings, {origin:config.cors, methods: verb})), null, {_middlewareType: 'CORS HOOK: setHeaders'});
            }
            ...
            // Else if cors is an object, use that as the config
              ...
              sails.router.bind(route, cors(_.extend({}, globalCorsModuleSettings, config.cors)), null, {_middlewareType: 'CORS HOOK: setHeaders'});
            }
        ...
        sails.router.bind('options /*', cors(setPreflightConfig(optionsRouteConfigs, globalCorsModuleSettings)), 'options', {_middlewareType: 'CORS HOOK: preflight'});
