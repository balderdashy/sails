module.exports = function(sails) {
        ...
        if (sails.config.cors.allRoutes === false) {
          sails.router.bind('/*', function(req, res, next) {
            ...
            if (req.isSocket) {
              ...
              if (res.headers && res.get('Access-Control-Allow-Origin')) {
                return next();
              }
              else {
  ...
  function getOrigin(socket) {
    if (socket.handshake.headers.host) return socket.handshake.headers.host;

    // If no `origin` exists on the socket, it is connected to the
    // same host, so we can assume it is the same as the host in
    // `sails.getBaseurl()`
    var host;
    try {
      // http://nodejs.org/docs/latest/api/url.html#url_url_parse_urlstr_parsequerystring_slashesdenotehost
      host = require('url').parse(sails.getBaseurl()).host;
    }
    catch (e) { host = 'localhost:1337'; }
