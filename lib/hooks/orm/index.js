...
module.exports = function(sails) {
	...
	return {
		...
		configure: function () {
			...
			if (sails.config.adapters) {
				...
				if (sails.config.adapters['default']) {
					sails.log.verbose('Deprecation warning :: Replacing `config.adapters.default` with `config.models.connection`....');
		...
		normalizeModelDef: function (modelDef, modelID) {
			...
			if ( ! util.isArray(modelDef.connection) ) {
			...
			modelDef.connection = util.map( modelDef.connection, function (connection) {
	 ...
	 *			e.g., `modelDef.adapter = util.cloneDeep(modelDef.connection);`
	 */
	function _buildAdHocAdapterSet (modelDefinitions) {
		...
		util.each(modelDefinitions, function eachModelDef (modelDef) {
			...
			util.each(modelDef.connection, function eachConnection (connection) {
			...
			// modelDef.adapter = util.pluck(modelDef.connection, 'adapter');
		});
