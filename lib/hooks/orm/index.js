module.exports = function(sails) {
	...
	return {
		...
		instantiateModels: function(cb) {
			...
			// Load model definitions into waterline
			var waterline = new Waterline();
			
			// Iterate through the connections for each model
			Object.keys(sails.models).forEach(function eachModelDef (modelID, modelDef) {
				sails.log.verbose('Registering model ' + modelID + '...');
				var Model = registerModel(modelID);
				waterline.loadCollection(Model);
			});

			// Pass adapters into waterline and kick things off
