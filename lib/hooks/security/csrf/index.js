module.exports = function(sails) {
  ...
  return function initializeCsrf() {
    ...
    sails.on('router:bind', checkForCSRFConfig);
    sails.on('route:typeUnknown', checkForCSRFConfig);

    function checkForCSRFConfig(routeDef) {

      // Get some info about the route, like its path and verb.
      var path = routeDef.path;
      var verb = (routeDef.verb || '').toLowerCase();

      var routeCsrfConfig = !_.isUndefined(routeDef.options.csrf) ? routeDef.options.csrf : routeDef.target.csrf;
      if (_.isUndefined(routeCsrfConfig)) {
        return;
      }
      ...
      if (routeCsrfConfig === false && sails.config.security.csrf === true) {
        blacklist.push({method: verb || '', regex: regex});
      }

      else if (routeCsrfConfig === true && sails.config.security.csrf === false) {
