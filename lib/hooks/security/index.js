  ...
  return {
    ...
    configure: function() {
      ...
      if (sails.config.security.cors.origin) {
        ...
        sails.log.error('(See http://sailsjs.com/config/security for more info.)'+'\n');
      ...
      // If the app attempts to set `origin: '*'` and `credentials: true`, bail out
      ...
      _.each(sails.config.routes, function(routeConfig, address) {
        ...
        // Deprecate `credentials` in favor of `allowCredentials`
        if (routeCorsConfig.credentials) {
          sails.log.warn('In route `' + ((verb ? (verb + ' ') : '') + path) + '`: ');
        ...
        // Deprecate `sails.config.cors.exposeHeaders` in favor of `sails.config.cors.allowResponseHeaders`
        if (routeCorsConfig.exposeHeaders) {
          sails.log.warn('In route `' + ((verb ? (verb + ' ') : '') + path) + '`: ');
          sails.log.error('The `cors.exposeHeaders` config has been deprecated.');
