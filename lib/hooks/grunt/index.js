   ...
   return {
     ...
     */
     runTask: function (taskName, cb_afterTaskStarted) {
      ...
      // Plugins Grunt file
      var grunt_tasks = sails.config.paths.plugins;
      grunt_tasks.push(sails.config.appPath);
      var child;

      _.forEach(grunt_tasks, function(taskPath){
        // check if exist gruntfile to run the process
        if (!fs.existsSync(path.resolve(taskPath, 'Gruntfile.js')) || !fs.existsSync(path.resolve(taskPath, 'Gruntfile.coffee'))) return;
