module.exports = function(sails) {
  ...
  return {
    ...
    initialize: function(cb) {
      ...
      var pathForEnvSpecificTaskList = path.resolve( sails.config.appPath, path.join('tasks/register/','build'+_.capitalize(sails.config.environment) + '.js') );
      if ( fs.existsSync(pathForEnvSpecificTaskList) ) {
        gruntTaskListName = 'build'+_.capitalize(sails.config.environment);
      }
      // If the environment is "production", use the "prod" tasklist (unless a "production" tasklist exists)
        ...
        gruntTaskListName = 'buildProd';
      }
      // If the environment is "staging", use the "staging" tasklist (unless a "staging" tasklist exists)
      else if (sails.config.environment === 'staging') {
        gruntTaskListName = 'buildStaging';
      }
      else {
        gruntTaskListName = 'build';
      }
