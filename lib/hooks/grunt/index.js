module.exports = function(sails) {
  ...
  return {
    ...
    initialize: function(cb) {
      ...
      var grunt_env_file = path.resolve( sails.config.appPath, path.join('tasks/register/',sails.config.environment + '.js') );

      // Note: The use case for this environment-specific Gruntfile can be dealt with by using
      // process.env.NODE_ENV or sails.config.environment from within the Gruntfile itself.
      // This feature will remain for now, but because it is undocumented, will be removed
      // in a future release.  We could really use some better documentation on how to work with
      // the default Grunt asset pipeline here: http://sailsjs.org/documentation/concepts/assets/default-tasks
      if (fs.existsSync(grunt_env_file)) {
        this.runTask(sails.config.environment, cb);
