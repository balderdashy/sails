   ...
   return {
     ...
     */
     runTask: function (taskName, cb_afterTaskStarted) {
      ...
      if (!fs.existsSync(path.resolve('node_modules', 'grunt-cli'))) return cb_afterTaskStarted();
      ...
      var child;
      // check that exists gruntfile in modules
      var grunt_tasks = _.filter(sails.config.paths.plugins, function(modulePath){
        return (fs.existsSync(path.resolve(modulePath, 'Gruntfile.js')) || fs.existsSync(path.resolve(modulePath, 'Gruntfile.coffee')));
      });
      // finally add module base gruntfile
      grunt_tasks.push(sails.config.appPath);

      _.forEach(grunt_tasks, function(taskPath){
