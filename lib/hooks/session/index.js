  ...
  var SessionHook = {
    ...
    defaults: {
      session: {
        ...
        //    while this construction of the session store does not require expressSessionFromSailsCore, some constructions will
        handleConstructingSessionStore: (sessionConfig, configuredSessionAdapter, expressSessionFromSailsCore) => { // eslint-disable-line no-unused-vars
      ...
      (function setupAdapter(proceed) {
        ...
        // For all other adapters, we'll try to require the module and do some setup.
          ...
          (function maybeConnectToRedis(proceed) {
            ...
            try {
              var CustomStore = app.config.session.handleConstructingSessionStore(app.config.session, SessionAdapter, require('express-session'));
            }
            catch (rawSessionStoreCreationErr) {

              // Failed attempting to initialize adapter; output a message w/ error info
              return proceed(new Error(
                'Encountered error attempting to instantiate a session store using the installed version of `' + app.config.session.adapter + '` (a session adapter) with the defined handleConstructingSessionStore function.\n'+
            ...
            app.config.session.store = CustomStore;
