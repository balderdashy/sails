  ...
  var SessionHook = {
    ...
    initialize: function(cb) {
      ...
      if (app.config.session.adapter === 'connect-redis' &&
          ...
          sails.config.session.store.client &&
          sails.config.session.store.client instanceof require('events').EventEmitter) {
        return (function() {
          // If the client is already connected (because it was provided in the config)
          // then just continue
          if (sails.config.session.store.client.connected) {return cb();}
          // Define an error handler for the Redis client which will stop Sails from lifting
          // by returning an error in the hook callback
          var redisErrorHandler = function(err) {
            return cb(err);
          };
          sails.config.session.store.client.on('error', redisErrorHandler);
          // If the Redis client successfully connects, cancel the error handler and continue
          sails.config.session.store.client.on('connect', function () {
            sails.config.session.store.client.removeListener('error', redisErrorHandler);
            return cb();
          });
        })();
      }
