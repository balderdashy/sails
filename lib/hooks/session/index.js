...
module.exports = function(app) {
  ...
  var SessionHook = {
    ...
    configure: function() {
      ...
      // Validate `disabledRoutes`, if specified.
      if (app.config.session.disabledRoutes && !_.isArray(app.config.session.disabledRoutes)) {
        throw new Error('Invalid `sails.config.session.disabledRoutes` configuration!\n' +
    ...
    initialize: function(cb) {
      ...
      // Build `sails.hooks.session.disabledRoutes`.
      // (only salient if `sails.config.session.disabledRoutes` was specified)
      try {

        sails.hooks.session.disabledRoutes = _.reduce(sessionConfig.routesDisabled || [], function (memo, routeAddressStr){
        ...
        return cb(
          new Error('Failed to parse one of the route addresses (`'+routeAddressStr+'`) provided in `sails.config.session.routesDisabled`.\n'+
