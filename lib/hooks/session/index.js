  ...
  var SessionHook = {
    ...
    initialize: function(cb) {
      ...
      if (app.config.session.adapter === 'connect-redis' &&
          ...
          app.config.session.store.client &&
          app.config.session.store.client instanceof require('events').EventEmitter) {
        return (function() {
          ...
          app.config.session.store.client.on('error', redisErrorHandler);
          // If the Redis client successfully connects, cancel the error handler and continue
          app.config.session.store.client.on('connect', function () {
            app.config.session.store.client.removeListener('error', redisErrorHandler);
            return cb();
          });
        })();
      }
