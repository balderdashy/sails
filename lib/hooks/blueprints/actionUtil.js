...
module.exports = {
  ...
  parseLimit: function (req) {
    ...
    var limit = req.param('limit') || (typeof req.options.limit !== 'undefined' || req.options.limit !== 0 ? req.options.limit : DEFAULT_LIMIT);
  ...
  parseSkip: function (req) {
    var DEFAULT_SKIP = 0;
    var skip = req.param('skip') || (typeof req.options.skip !== 'undefined' && req.options.skip !== 0 ? req.options.skip : DEFAULT_SKIP);
    if (isNaN(skip)) skip = DEFAULT_SKIP;
    if (skip) { skip = +skip; }
    return skip;
  }
};
