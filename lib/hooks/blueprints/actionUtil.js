...
module.exports = {
  ...
  populateEach: function ( query, req ) {
    ...
    return _(_options.associations).reduce(function populateEachAssociation (query, association) {
      ...
      if (shouldPopulate) {
        ...
        var criteria = {limit: populationLimit}
          , where;

        // Was the association alias supplied as one of the params, and is it not a string?
        if ((where = req.param(association.alias)) && typeof where === 'object') {
          _.merge(criteria, self.parseCriteria(req, where));
        }

        return query.populate(association.alias, criteria);
      }
      else return query;
    }, query);
  },
   ...
   */
  parseCriteria: function ( req, where ) {
    ...
    where = where || req.params.all().where;
