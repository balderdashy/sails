...
var actionUtil = {
  ...
  */
  ...
  parseCriteria: function ( req ) {
    ...
    //
    // > See https://github.com/balderdashy/waterline-criteria#validatewhereclause.
  ...
  parseLimit: function (req) {
    var defaultLimit = req._sails.config.blueprints.defaultLimit || 30;
    // An optional ceiling for the `limit`.
    // (if exceeded, limit will be set to maxLimit)
    var maxLimit;
    if (!_.isUndefined(req.options.maxLimit)) {
      maxLimit = req.options.maxLimit;
    }
    else if (!_.isUndefined(sails.config.blueprints.maxLimit)) { 
      maxLimit = sails.config.blueprints.maxLimit;
    }
    var limit = req.param('limit') || (typeof req.options.limit !== 'undefined' ? req.options.limit : defaultLimit);
