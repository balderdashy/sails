...
module.exports = {
  ...
  parseCriteria: function ( req ) {
    ...
    //Ensure there is a criteria object.
    ...
    // Allow customizable whitelist to exclude any excess params.
    req.options.criteria.whitelist = req.options.criteria.whitelist;

    //Validate whitelist to provide a more helpful error message.
    var whitelist = req.options.criteria.whitelist;
    ...
    if (!where) {
      ...
      // Parse out values not on whitelist
  ...
  parseValues: function (req) {
    ...
    // Validate blacklist to provide a more helpful error message.
