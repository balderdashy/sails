...
module.exports = function(req) {
  ...
  switch (blueprint) {
    ...
    case 'findOne':
      ...
      if (!_.isUndefined(req.param('limit'))) { queryOptions.criteria.limit = req.param('limit'); }
    ...
    case 'add':
      ...
      queryOptions.childPk = req.param('childid');
    ...
    case 'replace':
      ...
      queryOptions.childPks = _.isArray(req.body) ? req.body : req.query[req.options.alias];

      if (_.isString(queryOptions.childPks)) {
        try {
          queryOptions.childPks = JSON.parse(queryOptions.childPks);
        } catch (e) {
          return res.badRequest(new Error('Invalid values for `' + req.options.alias + '` collection given to "replace" blueprint action.  If specified as a string, the value must be parseable as a JSON array, e.g. "[1,2]".'));
        }
      }
    ...
    case 'populate':
      ...
      queryOptions.populates[req.options.alias] = (function getPopulateCriteria(){
      ...
      if (!_.isUndefined(req.param('limit'))) { queryOptions.populates[req.options.alias].limit = req.param('limit'); }
