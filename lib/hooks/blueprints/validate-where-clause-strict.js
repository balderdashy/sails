    ...
    _.each(clause, function (rhs, key){
      ...
      // Else if this is a sub-attribute modifier...
        ...
        // Otherwise the RHS is supposed to be an equivalency filter.
          ...
          // If this sub-attribute modifier is specific to strings (e.g. "contains")
          // then only allow strings, numbers, and booleans.
          if (_.contains(STRING_SEARCH_MODIFIERS, key)) {
            if (!_.isString(rhs) && !_.isNumber(rhs) && !_.isBoolean(rhs)){
              throw flaverr('E_WHERE_CLAUSE_UNPARSEABLE', new Error(E_WHERE_CLAUSE_UNPARSEABLE_MSG_PREFIX + 'Unexpected value at `'+key+'`:'+util.inspect(rhs,{depth: null})+'\n(The right-hand side of a string search modifier like `'+key+'` must always be a string, number, or boolean.)'));
            }
          }
          if (!isEqFilter(rhs)) {
