...
module.exports = function update(req, res) {
	...
	if (isJSONPCompatible) {
		data = sails.util.omit(data, JSONP_CALLBACK_PARAM);
	}
	...
	Model.findOne(id).exec(function(err, matchingRecord) {
		...
		Model.update(id, data).exec(function updated(err, models) {
			...
			if (err && err.code === 'E_VALIDATION') return res.badRequest(err);
			if (err && err.code === 'E_CONSTRAINT') return res.badRequest(err);
			if (err) return res.serverError(err);
			...
			if (sails.hooks.pubsub) {
				...
				Model.publishUpdate(id, pubData, !req.options.mirror && req, {
					previous: matchingRecord.toJSON()
				});
			}

			var Q = Model.findOne(model.id);
			Q = actionUtil.populateAll(Q, req.options);
			Q.exec(function found(err, refoundRecord) {
				if (err) return res.serverError(err);

				if (isJSONPCompatible) {
					return res.jsonp(refoundRecord);
				} else {
					return res.json(refoundRecord);
				}
			});
