  ...
  async.auto({
    ...
    notificationsForExistingChildrenOfParent: function(cb) {
      ...
      if (!inverseAttr || inverseAttr.collection) {
        ...
        populateCriteria.where[relatedModel.primaryKey] = {'nin': [childPks]};

        Model.findOne({id: parentPk}).populate(relation, populateCriteria).exec(function(err, parentRecord) {
      ...
      criteria.where[relatedModel.primaryKey] = {nin: childPks};
