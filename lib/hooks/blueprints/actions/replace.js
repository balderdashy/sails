  ...
  async.auto({
    ...
    Model.replaceCollection(parentPk, relation, childPks).exec( function(err) {
      ...
      if (err) {
        ...
        // ```
        // return res.status(409).badRequest(err);
        // ```
        // - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        switch (err.name) {
          case 'UsageError': 
              switch(err.code) {
              // If there is a uniqueness error, then this collection is being modified by a different process
              // that is editing records at the same time-- uniqueness errors cannot happen because of duplicates 
              // in the array of child ids. For now, we'll respond with a 500 error in this case, but see the note 
              // above about how this will be handled in the future.
