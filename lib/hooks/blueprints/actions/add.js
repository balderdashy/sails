...
module.exports = function addToCollection (req, res) {
  ...
  async.auto({
    ...
    actualChildPkValue: ['parent', function(cb) {
      ...
      // the primary key attribute, so we can pull out the primary key value
      ...
      if (child[childPkAttr]) {
        ChildModel.findOrCreate(child[childPkAttr], child).exec(function foundOrCreatedNewChild (err, childRecord){
          if (err) return cb(err);
