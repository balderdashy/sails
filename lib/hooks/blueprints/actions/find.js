...
module.exports = function find (req, res) {
	...
	// The query we'll be building up below
	var Q;
	...
	if (req.param('id')) {
		...
		Q = actionUtil.populateAll(Q, req.options);
		Q.exec(function found(err, matchingRecord) {
			...
			// about this instance IF pubsub is enabled AND this is a socket request
			if (sails.hooks.pubsub && req.isSocket) {
				Model.subscribe(req, matchingRecord);

				// Also subscribe to instances of all associated models
				subscribeDeep(req.options.associations, matchingRecord);
			}
	 ...
	 */
		...
		Q.exec(function found(err, matchingRecords) {
			...
			// Use the model class's subscribe method to subscribe to all blueprint notifications 
			// about these instances IF pubsub is enabled AND this is a socket request
			if (sails.hooks.pubsub && req.isSocket) {
				Model.subscribe(req, matchingRecords);

				// Only `.watch()` for new instances of the model if `autoWatch`
				// is enabled.
				...
				_.each(matchingRecords, function (record) {
					subscribeDeep(req.options.associations, record);
				});
			}
	 ...
	 */
	function subscribeDeep (associations, record) {
