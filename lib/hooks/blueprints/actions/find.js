...
module.exports = function find (req, res) {
	...
	if (req.param('id')) {
		...
		Q = _populateAssociations(Q, req.options);
 ...
 * @return {Query}
 */
function _populateAssociations ( query, options ) {
	return _(options.associations).reduce(function (query, association) {
	...
	// false to not)
	if (options.populate){
		return query.populate(association.alias /*, { limit: 30 } */);
	}
	else return query;
	}, query);
}
