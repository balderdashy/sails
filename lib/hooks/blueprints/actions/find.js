...
module.exports = function find (req, res) {
	...
	var JSONP_CALLBACK_META_PARAM = '_jsonpCallbackParam';
	var jsonpCallbackParam = req.param(JSONP_CALLBACK_META_PARAM) || req.options.jsonpCallbackParam || 'callback';
	 ...
	 */
		...
		Q = _(associations).reduce(function (Q, association) {
			// console.log('populating '+association.alias);
			return Q.populate(association.alias /*,  { limit: 30 } */);
		}, Q);
		// if (associations[0] && associations[0].alias) {
		// 	Q.populate('dad');
		// }
		Q.exec(function found(err, matchingRecords) {
			...
			// Otherwise serve a JSON(P) API
			if ( isJSONPCompatible ) {
				res.jsonp(matchingRecords);

				// Revert the JSONP callback param to what it was before
				req.app.set('jsonp callback name', originalJsonpCallbackParam);
				return;
			}
			else {
	...
	function parseWhereParam( allParams ) {
		...
		if (!where) {
			...
			if (isJSONPCompatible) { where = sails.util.omit(where, [jsonpCallbackParam]); }
			where = sails.util.omit(where, [JSONP_CALLBACK_META_PARAM]);
			// console.log(JSONP_CALLBACK_META_PARAM);
			// console.log(isJSONPCompatible, jsonpCallbackParam, 'hi');
