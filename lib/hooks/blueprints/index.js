module.exports = function(sails) {
	 ...
	 * (This array concatenation approach is crucial to allow policies to be bound)
	 * 
	 * @param  {String} controllerId 
	 * @param  {String} actionId     
	 * @return {[Function]}          array of 2 middleware- configurator + the blueprint itself
	 */
	...
	function bindShadowRoutes () {
		...
		_.each(sails.controllers, function eachController (controller, controllerId) {
			...
			_.each(actions, function eachActionID (actionId) {
				var actionHandler = _serveBlueprint(controllerId, actionId);
			...
			if ( conf.blueprints.rest ) {
				...
				sails.router.bind('get ' + baseRoute + '/:id?', _serveBlueprint(controllerId, 'find'));
				sails.router.bind('post ' + baseRoute, _serveBlueprint(controllerId, 'create'));
				sails.router.bind('put ' + baseRoute + '/:id?', _serveBlueprint(controllerId, 'update'));
				sails.router.bind('delete ' + baseRoute + '/:id?', _serveBlueprint(controllerId, 'destroy'));
			}
			...
			if ( conf.blueprints.shortcuts ) {
				...
				sails.router.bind(baseRoute + '/find/:id?', _serveBlueprint(controllerId, 'find'));
				sails.router.bind(baseRoute + '/create', _serveBlueprint(controllerId, 'create'));
				sails.router.bind(baseRoute + '/update/:id?', _serveBlueprint(controllerId, 'update'));
				sails.router.bind(baseRoute + '/destroy/:id?', _serveBlueprint(controllerId, 'destroy'));
			}
		});
	}
