  ...
  return {
    ...
    defaults: {
      ...
      blueprints: {
        ...
        // Default limit for find blueprint and for populates.
        defaultLimit: 30,
    ...
    configure: function() {
      ...
      if ((!_.isUndefined(sails.config.blueprints.defaultLimit) && parseInt(sails.config.blueprints.defaultLimit) !== sails.config.blueprints.defaultLimit) || sails.config.blueprints.defaultLimit < 1) {
        throw new Error('If `sails.config.blueprints.defaultLimit` is provided, it must be a non-negative integer.');
      }
