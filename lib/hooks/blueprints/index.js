		...
		_.each(sails.middleware.controllers, function eachController (controller, controllerId) {
			...
			if ( config.prefix ) {
				if ( !_(config.prefix).isString() ) {
					console.log();
					sails.log.warn(util.format('Invalid blueprint prefix configured for controller `%s`.', controllerId));
					sails.log.warn('`prefix` should be a string, e.g. "/api/v1".');
					sails.log.warn('For more information, see `http://links.sailsjs.org/docs/config/blueprints`');
					return;
				}
				if ( !config.prefix.match(/^\//) ) {
					console.log();
					sails.log.warn(util.format('Invalid blueprint prefix configured for controller `%s`.', controllerId));
					sails.log.warn(util.format('Should start with a `/`, like "/api/v1", not "%s"...but I\'m assuming you meant:  "%s".', config.prefix, '/'+config.prefix));
					sails.log.warn('For more information, see `http://links.sailsjs.org/docs/config/blueprints`');
			...
			_.each(actions, function eachActionID (actionId) {
				...
				if (config.actions) {
					...
					sails.router.bind(actionRoute, controller[actionId], null, opts);
				}
			...
			if (sails.hooks.orm && sails.models && sails.models[controllerId]) {
				...
				if ( config.shortcuts ) {
					...
					_(Model.associations).where({type: 'collection'}).forEach(function (association) {
						...
						var opts = _.defaults({ alias: alias }, routeOpts);
