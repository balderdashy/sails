		...
		_.each(sails.middleware.controllers, function eachController (controller, controllerId) {
			...
			if (sails.hooks.orm && sails.models && sails.models[controllerId]) {
				...
				if ( config.rest ) {
					...
					// Bind resourceful routes based on known relations in our schema
					Model.associations = {
						friends: { collection: 'user' }
					};
					_.each(Model.associations, function eachAssociation (association, alias) {
						var relativeId, route, action;

						// Figure out the identity and bind a blueprint route for this association.
						
						// [m..n] assocation
						if (association.collection) {
							relativeId = association.collection;
							route = 'get ' + baseRoute + '/:parentid/' + alias + '/:id?';
						}
						// [m..1] assocation
						else if (association.model) {
							relativeId = association.model;
							route = 'get ' + baseRoute + '/:parentid/' + alias;
						}
						console.log('Binding blueprint route for association `'+alias+'` for',controllerId, route);
						
						sails.router.bind(route, _getMiddlewareForShadowRoute(controllerId, 'expand'), null, _.merge({}, routeOpts, {
							alias: alias
						}));
						return;
					});
