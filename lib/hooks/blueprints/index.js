      ...
      if ( config.rest ) {
        ...
        _.each(sails.models, function(Model, identity) {
          ...
          function _bindRestRoute(template, blueprintActionName) {
            ...
            sails.router.bind(restRoute, identity + '.' + blueprintActionName, null, { model: identity });
          }

          function _bindAssocRoute(template, blueprintActionName, alias) {
            var assocRoute = util.format(template, baseRestRoute, alias);
            sails.router.bind(assocRoute, identity + '.' + blueprintActionName, null, { model: identity, alias: alias });
          }
      ...
      if ( config.shortcuts ) {
        ...
        _.each(sails.models, function(Model, identity) {
          ...
          function _bindShortcutRoute(template, blueprintActionName) {
            ...
            sails.router.bind(shortcutRoute, identity + '.' + blueprintActionName, null, { model: identity });
          }
