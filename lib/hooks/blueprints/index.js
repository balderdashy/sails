...
var _          = require('lodash'),
    ...
    LANGS      = require('../i18n')().defaults.i18n.locales;

    /**
     * Prepare langs baseroute
     * Finally adding default baseroute without lang
     */
  ...
  return {
    ...
    bindShadowRoutes: function() {
      ...
      _.each(sails.middleware.controllers, function eachController (controller, controllerId) {
        ...
        // Determine base route for each language
        _(LANGS).forEach(function(lang) {
          var baseRoute = config.prefix + lang + controllerId;
          ...
          if (sails.hooks.orm && sails.models && sails.models[modelId]) {
            ...
            if ( config.shortcuts ) {
              ...
              sails.router.bind( _getRoute('%s/find'),           _getAction('find'),    null, routeOpts );
              sails.router.bind( _getRoute('%s/find/:id'),       _getAction('findOne'), null, routeOpts );
              ...
              _(Model.associations).where({type: 'collection'}).forEach(function (association) {
                ...
                sails.router.bind( _getAssocRoute('%s/:parentid/%s/remove/:id?'),   _getAction('remove'), null, opts );
              });
            }
            ...
            if ( config.rest ) {
              ...
              sails.router.bind( _getRoute('get %s'),         _getAction('find'),    null, routeOpts );
              sails.router.bind( _getRoute('get %s/:id'),     _getAction('findOne'), null, routeOpts );
