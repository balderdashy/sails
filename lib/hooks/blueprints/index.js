			...
			if (sails.hooks.orm && sails.models && sails.models[controllerId]) {
				...
				if ( config.shortcuts ) {
					...
					Model.associations.forEach(function (association) {
						...
						sails.log.silly('Binding shortcut blueprint/shadow routes for association `'+alias+'` for',controllerId);
						sails.router.bind( _getAssocRoute('%s/:parentid/%s'),      _getAction('add'), null, routeOpts );
						sails.router.bind( _getAssocRoute('%s/:parentid/%s'),      _getAction('remove'), null, routeOpts );
					});
				}
				...
				if ( config.rest ) {
					...
					Model.associations.forEach(function (association) {
						...
						sails.router.bind( _getAssocRoute('post %s/:parentid/%s'),     _getAction('add'), null, routeOpts );
						sails.router.bind( _getAssocRoute('delete %s/:parentid/%s'),   _getAction('remove'), null, routeOpts );
					});
				}
