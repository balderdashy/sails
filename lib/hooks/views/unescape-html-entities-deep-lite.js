...
module.exports = function unescapeHtmlEntitiesDeepLite(data){
  ...
  return (function _unescapeRecursive (thisVal) {
    ...
    // If this is a string, then unescape it.
      ...
      //
      // Define mapping between HTML entities and their decoded, real-world characters.
      var htmlUnescapes = {
        '&amp;': '&',
        '&lt;': '<',
        '&gt;': '>',
      ...
      if (thisVal === '') {
        return thisVal;
      }//<< maybe not necessary; TODO test this
      else if (!X_HAS_ESCAPED_HTML.test(thisVal)) {
        return thisVal;
      }
      else {
        return thisVal.replace(X_ESCAPED_HTML, basePropertyOf(htmlUnescapes));
      }
    }
    // ‡
