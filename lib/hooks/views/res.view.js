  ...
  res.view = function(/* specifiedPath, locals, cb_view */) {
    ...
    if (_.isUndefined(res.locals.exposeLocalsToBrowser)) {
      res.locals.exposeLocalsToBrowser = function (options){
        // We provide access to `res.locals` using a reference
        // obtained via closure-- and this view helper won't be
        // used until AFTER the rest of the code in THIS file
        // has run-- so we know all of the other locals data
        // attached below will be available, since we're referring
        // to the same object.
