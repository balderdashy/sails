...
function getEngine(appDependenciesPath,engineName) {
  ...
  if(!consolidate.requires[engineName]){
    try{
      //ensure the engine is required relative to the path of our app
      consolidate.requires[engineName] = require(appDependenciesPath+'/' + engineName);
    }catch(e){
      console.log(['Could not find module:',engineName,"in path:",appDependenciesPath].join(" "));
    }
  }
