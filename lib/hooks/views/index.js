module.exports = function (sails) {
	...
	function _addResViewMethod(req, res, next) {
		...
		res.view = function (specifiedPath, locals, cb_view) {
			...
			if (layout === undefined && typeof res.locals.layout !== 'undefined' ) {
				layout = res.locals.layout;
			}

			// if local `layout` is set to true or is not specified explicitly or in
			// res.locals, then fall back to global config
