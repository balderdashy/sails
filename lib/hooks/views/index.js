module.exports = function (sails) {
	...
	return {
        ...
        if(sails.config.modules) {
          util.forEach(sails.config.modules.register, function(value) {
              self.glob(sails.config.modules.path + '/' + value + '/views', function(err, moduleViews){
                if(err) {
                  console.error(' - Views for module "' + value +  '" not loaded!');
                }
                else {
                  detectedViews[value] = moduleViews;
                  console.log(' - Views for module "' + value + '" loaded.');
                }
              });
            }
          );
...
//				var relPathToLayout = path.relative( path.dirname(absPathToView), absPathToLayout);
				...
				res.locals._layoutFile = sails.config.paths.layout;//relPathToLayout;
			}
