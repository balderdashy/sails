module.exports = function (sails) {
	...
	return {
		...
		initialize: function (cb) {
			...
			sails.on('route:typeUnknown', bindRoute);
	...
	function bindRoute(route) {
		if (!route.target || !(util.isString(route.target) || route.target.controller || route.target.view)) {
			return;
		}
		
		var view = null, subview = null, controllerId = null;
		if (util.isString(route.target)) {
			...
			// Find controller
			view = route.target.controller;
			controllerId = route.target.controller;
			subview = route.target.action;
		} else {
			return;
		}
		...
		if (controllerId) {
			...
			if (controller && controller[subview]) {
				// Let controller middleware handle it
				return;
			}
		}
		...
		sails.router.bind(route.path, viewMiddleware, route.verb);
	}
	...
	function _addResViewMethod(req, res, next) {
		...
		/**
		 * res.view(specifiedPath, [data], [cb_view])
		 *
		 * 
		 */

		res.view = function (specifiedPath, data, cb_view) {
			...
			// Render the view
			return res.render(relPathToView, data, function (err, renderedViewStr) {
				if (err) {
				...
				if (cb_view) { cb_view(err); }

				// Extend locals with data passed in
				util.extend(res.locals, data);
