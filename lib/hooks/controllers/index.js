	...
	return {
		...
		initialize: function(cb) {
			...
			this.loadMiddleware(function (err) {
				...
				_.each(blueprints, function (ware, route) {
					...
					if ( blueprintsEnabled[route] ) {
						...
						// Set the default blueprint up to be bound
						self.routes.after[actualRoute] = ware;

						// If the user overrode the blueprint ware,
						// set the custom method up to be bound
						if ( _.isFunction (blueprintsEnabled[route]) ) {
							self.routes.after[actualRoute] = blueprintsEnabled[route];
						}
