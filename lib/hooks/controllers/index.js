module.exports = function(sails) {
	...
	return {
      ...
      sails.controllers = {};
      this.loadControllers(sails.config.paths.controllers, '', function(err) {
        if(err) return cb(err);
        if(sails.config.modules) {
          async.forEach(sails.config.modules.register, function(value, next) {
            self.loadControllers(sails.config.modules.path + '/' + value + '/controllers', value + '/', function(err){
              if(err) {
                console.error(' - Controllers for module "' + value +  '" not loaded!');
              }
              else {
                console.log(' - Controllers for module "' + value + '" loaded.');
              }
    ...
    loadControllers: function(path, prefix, cb) {
      var self = this;
      Modules.optional({
        dirname: path,
        ...
        _.each(modules, function(controller, controllerId) {flattenController(prefix + controllerId, controller);});

        // Save freshly loaded modules in `sails.controllers`
        util.merge(sails.controllers, controllers);
        ...
        _.each(controllers, function(controller, controllerId) { //sails.
          var cID = controllerId;
          // Override whatever was here before
          if ( !util.isDictionary(self.middleware[cID]) ) {
            self.middleware[cID] = {};
          }

          // Mix in middleware from blueprints
          self.middleware[cID].find = Controller.find;
          self.middleware[cID].create = Controller.create;
          self.middleware[cID].update = Controller.update;
          self.middleware[cID].destroy = Controller.destroy;
          ...
          _.each(controller, function(action, actionId) {
            ...
            if (action === false) {
              delete self.middleware[cID][actionId];
              return;
            }
            ...
            self.middleware[cID][actionId] = action;
          });
        });
