module.exports = function(sails) {
	 ...
	 * helper functions
	 */
var mergeBlueprints = function (configBP, controllerBP) {
	var deepMerge = util.deepMerge;
	var finalBlueprints = configBP; 
	//support blueprint config on a per-controller basis
	if (controllerBP){
		finalBlueprints = deepMerge(configBP,controllerBP);
	}
	
	return finalBlueprints;
};
var getBlueprintsPrefix = function (blueprints) {
	var blueprintsPrefix = blueprints.prefix;
	// Trim trailing slash 
	if (blueprintsPrefix[(blueprintsPrefix.length - 1)] === '/') {
		blueprintsPrefix = blueprintsPrefix.slice(0, -1);
	}
	if (blueprintsPrefix[0] === '/') {
		blueprintsPrefix = blueprintsPrefix.slice(1,blueprintsPrefix.length);
	}
	if(blueprintsPrefix.length){
		blueprintsPrefix = '/' + blueprintsPrefix;
	}
		...
		bindActionRoutes: function () {
			...
			if ( sails.config.controllers.blueprints.actions ) {
				...
				_.each(sails.controllers, function (controller, controllerId) {
					...
					var finalBlueprints = mergeBlueprints(sails.config.controllers.blueprints, controller.blueprints);
					var blueprintsPrefix = getBlueprintsPrefix(finalBlueprints);
					
					if (!finalBlueprints.actions) {
						return;
					}
