module.exports = function(sails) {
	...
	return {
		...
		bindRoute: function (ware, route) {
			...
			var blueprintsRouteConfig = sails.config.controllers.blueprints.routes;
			...
			actualRoute = util.detectVerb(actualRoute).path;
			...
			// sails.log('binding route: ', route, actualRoute, blueprintsPrefix);
			
			// If the blueprint is enabled, we must prepare to bind a route
			if ( blueprintsRouteConfig[route] ) {
				
				// If the user overrode the blueprint ware,
				// set the custom method up to be bound
				if ( _.isFunction (blueprintsRouteConfig[route]) ) {
					ware = blueprintsRouteConfig[route];
				}
