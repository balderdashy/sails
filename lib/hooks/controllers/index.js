	...
	return {
		...
		bindActionRoutes: function () {
			...
			var blueprintsPrefix = sails.config.controllers.blueprints.prefix,
				pluralizeControllers = sails.config.controllers.blueprints.pluralize;
			...
			if ( sails.config.controllers.blueprints.actions ) {
				...
				_.each(sails.controllers, function (controller, controllerId) {
					...
					var controllerActions = _.functions(sails.controllers[controllerId]),
						controllerRoute = pluralizeControllers ? pluralize(controllerId) : controllerId;
					_.each(controllerActions, function (actionId) {
						...
						// Bind index route
						if (actionId === 'index') {
							var indexRoute = blueprintsPrefix + '/' + controllerRoute;
						...
						var appliedRoute = blueprintsPrefix + '/' + controllerRoute + '/' + actionId + '/:id?';
		...
		bindCRUDRoutes: function ( ) {
			...
			_.each(sails.controllers, function (controller, controllerId) {
				...
				var controllerRoute = pluralizeControllers ? pluralize(controllerId) : controllerId;
				...
				if ( sails.config.controllers.blueprints.rest ) {
					...
					if (sails.config.controllers.blueprints.jsonAPI) {
						updateVerb = 'patch';
						sails.router.bind('get ' + blueprintsPrefix + '/'+ controllerRoute + '/:ids([^,]+,[^,]+)?', _serveBlueprint(controllerId, 'find'));
					}

					sails.router.bind('get ' + blueprintsPrefix + '/'+ controllerRoute + '/:id?', _serveBlueprint(controllerId, 'find'));
					sails.router.bind('post ' + blueprintsPrefix + '/'+ controllerRoute, _serveBlueprint(controllerId, 'create'));
					sails.router.bind(updateVerb + ' ' + blueprintsPrefix + '/'+ controllerRoute + '/:id?', _serveBlueprint(controllerId, 'update'));
					sails.router.bind('delete ' + blueprintsPrefix + '/'+ controllerRoute + '/:id?', _serveBlueprint(controllerId, 'destroy'));
				}
				...
				if ( sails.config.controllers.blueprints.shortcuts ) {
					...
					sails.router.bind(blueprintsPrefix + '/'+ controllerRoute + '/find/:id?', _serveBlueprint(controllerId, 'find'));
					sails.router.bind(blueprintsPrefix + '/'+ controllerRoute + '/create', _serveBlueprint(controllerId, 'create'));
					sails.router.bind(blueprintsPrefix + '/'+ controllerRoute + '/update/:id?', _serveBlueprint(controllerId, 'update'));
					sails.router.bind(blueprintsPrefix + '/'+ controllerRoute + '/destroy/:id?', _serveBlueprint(controllerId, 'destroy'));
				}
			});
