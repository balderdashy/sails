...
module.exports = function(sails) {
  ...
  return {
    ...
    initialize: function(cb) {
      ...
      // Handle hook reload event
      sails.on('hook:controllers:reload', function(actions) {
        self.reload(actions);
      });
    ...
    reload: function(actions) {
      ...
      // If we received an explicit set of actions to load, use them.
      if (actions) {
        sails.hooks.controllers._actions = actions;
        return;
      }

      // Otherwise reload actions from disk.
      sails.hooks.controllers.loadModules(function(err){
        if (err) {throw err;}
      });
    ...
    loadModules: function(cb) {
      ...
      var loadAction = require('./load-action')(sails);
      ...
      includeAll.optional({
        ...
        try {
          ...
          _.each(files, function(module) {
            ...
            else if ((match = actionRegex.exec(filePath)) && (!_.isUndefined(module.machine) || !_.isUndefined(module.friendlyName) || _.isFunction(module.fn))) {
