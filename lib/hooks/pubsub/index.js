...
var NOTICES = {
	'classroom': 'Please use instance rooms instead (or raw sails.sockets.*() methods.)'
};
	...
	function addLowLevelSocketMethods() {
		...
		sails.sockets.join = function(socket, roomName, cb) {
			...
			if (typeof socket == 'string') {
				socket = sails.io.sockets.socket(socket);
			}
		...
		sails.sockets.socketRooms = function(socket) {
			return _.map(_.keys(sails.io.sockets.manager.roomClients[socket.id]), function(roomName) {
	...
	function getPubsubMethods() {
		...
		return {
			...
			publish: function(models, message, data, req) {
				...
				if (!models) {
					logDeprecationNotice(
						'Model.publish(null, ...)',
						LINKS.docs.sockets.pubsub,
						sails.log.debug) &&
						logUpgradeNotice(NOTICES.classroom, [], sails.log.debug);
			...
			subscribe: function(req, models, cb) {
				...
				cb = cb || function optionalCallback(err) {
