  ...
  function getPubsubMethods () {
    ...
    return {
      ...
      publish: function(ids, data, req) {
        ...
        // ids is required -- use `.watch()` to subscribe to the 'class room'.
        if (!ids) {
        ...
        if (!_.isArray(ids) || _.any(ids, function(id) {return !_.isString(id) && !_.isNumber(id);})) {
          sails.log.warn('`' + self.identity + '.publish`: `ids` argument should be an array of primary keys! Instead, got: ');
          sails.log.warn(util.inspect(ids, {depth: null}));
          ids = _.pluck(this.pluralize(ids), this.primaryKey);
        }
