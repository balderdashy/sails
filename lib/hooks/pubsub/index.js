    ...
    return {
      ...
      */
      subscribe: function (req, ids) {
        ...
        if (!_.isArray(ids) || _.any(ids, function(id) {return !_.isString(id) && !_.isNumber(id);})) {
          ids = _.pluck(pluralize.apply(this, [ids]), this.primaryKey);
          sails.log.warn('The second argument passed to `' + self.identity + '.subscribe()` must be an array of ids.  To subscribe to a single record, wrap the id in an array.');
          sails.log.warn('For example: `[' + ids[0] + ']`');
          sails.log.warn('Wrapping it in an array for you this time...');
        }
      ...
      unsubscribe: function (req, ids) {
        ...
        if (!_.isArray(ids) || _.any(ids, function(id) {return !_.isString(id) && !_.isNumber(id);})) {
          ids = _.pluck(pluralize.apply(this, [ids]), this.primaryKey);
          sails.log.warn('The second argument passed to `' + self.identity + '.unsubscribe()` must be an array of ids.  To subscribe to a single record, wrap the id in an array.');
          sails.log.warn('For example: `[' + ids[0] + ']`');
          sails.log.warn('Wrapping it in an array for you this time...');
        }
        ...
        _.each(values, function(val, key) {
          ...
          if (association.type === 'model') {
            ...
            reverseAssociation = _.find(ReferencedModel.associations, {collection: this.identity, via: key}) || _.find(ReferencedModel.associations, {model: this.identity, via: key});
              ...
              // - - - - - - - - - - - - - - - - - - - - - - - - -
              // TODO -- rip out all support for nested creates.
              // - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      _watch: function ( req ) {
        ...
        if (!socket) {
          sails.log.warn('`Model._watch()` called by a non-socket request. Only requests originating from a connected socket may be subscribed. Ignoring...');
          return;
        }//-•
