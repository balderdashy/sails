					...
					_.each(this.associations, function(association) {
						...
						if (association.type == 'model' && [association.alias]) {
							...
							if (reverseAssociation.type == 'model') {
								...
								ReferencedModel.publishUpdate(previous[association.alias].id, pubData, req);
							} 
							// If it's a to-many, publish a "removed" alert
							else {
								ReferencedModel.publishRemove(previous[association.alias].id, reverseAssociation.alias, id, req);
							}
						}
