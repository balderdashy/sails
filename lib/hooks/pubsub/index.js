	...
	return {
		...
		resume: function(cb) {
			...
			for (var identity in sails.models) {
				...
				util.bindAll(sails.models[identity], 
					'subscribe',
					'introduce',
					'obituary',
					'unsubscribe',
			...
			*/
			...
			publish: function (models, message, socketToOmit) {
				...
				function broadcast(room) {
					sails.log.verbose("Published "+message+" to "+self.classRoom());
					if (socketToOmit && socketToOmit.broadcast) {
						sails.log.verbose('(omitting socket #'+socketToOmit.id+' from broadcast...)');
						try {
							socketToOmit.broadcast.to( self.classRoom() ).json.send(message);
							return;
						}
						catch (e) {}
					}
			...
			*/
			...
			subscribe: function (socket, models) {
				...
				if (!models) {
					...
					sails.log.verbose("Subscribed to collection :: "+self.classRoom());
					return;
				}
				...
				util.each(ids,function (id) {
					sails.log.verbose("Subscribed to model :: "+self.room(id));
					socket.join( self.room(id) );
				});
			},
