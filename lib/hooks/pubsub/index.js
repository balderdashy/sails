            ...
            if (association.type == 'model') {
              ...
              reverseAssociation = _.find(ReferencedModel.associations, {collection: this.identity, via: key}) || _.find(ReferencedModel.associations, {model: this.identity, via: key});
                ...
                if (reverseAssociation.type == 'collection') {
                  ...
                  if (previous[key]) {
                    ReferencedModel.publishRemove(previous[key][ReferencedModel.primaryKey], reverseAssociation.alias, id, {noReverse:true});
                  }
                  // If there's a new value (i.e. it's not null), alert the newly associated model
                  if (val) {
                    ReferencedModel.publishAdd(_.isObject(val) ? val[ReferencedModel.primaryKey] : val, reverseAssociation.alias, id, {noReverse:true});
                  }
                }
                // Otherwise do a publishUpdate
                  ...
                  if (previous[key]) {
                    ...
                    ReferencedModel.publishUpdate(previous[key][ReferencedModel.primaryKey], pubData, req, {noReverse:true});
                  }
                  // If there's a new association, notify it that it has been linked
                  if (val) {
                    pubData[reverseAssociation.alias] = id;
                    ReferencedModel.publishUpdate(_.isObject(val) ? val[ReferencedModel.primaryKey] : val, pubData, req, {noReverse:true});
                  }
