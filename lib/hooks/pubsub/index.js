module.exports = function(sails) {
	...
	function pubsub () {
		...
		return {
			...
			*
			* @param {Socket} socketToOmit - if specified, use this 
			* socket to broadcast the message (effectively omitting it)
			*/
			...
			publish: function (models, message, socketToOmit) {
				...
				if (!models) {
					...
					if (socketToOmit) {
						try {
							socketToOmit.broadcast.to( my.classRoom() ).json.send(message);
							return;
						}
						catch (e) {}
					}
			...
			publishDestroy: function (values, socketToOmit) {
			...
			*/
			subscribe: function (req, models) {
				if (req.transport !== 'socket.io') return;
				var my = this;
