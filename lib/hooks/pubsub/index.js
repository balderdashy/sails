...
module.exports = function(sails) {
  ...
  function getPubsubMethods () {
    ...
    return {
      ...
      publish: function(records, event, data, req) {

        var self = this;

        records = this.pluralize(records);
        ...
        if (!_.isString(event)) {
          sails.log.error('`' + this.identity + ':publish` :: event must be specified as a string.');
          return;
        }
        ...
        _.each(ids, function(id) {
          var room = self.room(id, event);
          self.broadcast( room, event, data, socketToOmit );
        });
      ...
      _publishRPS: function (records, data, req) {
        ...
        // Loop through the record IDs to broadcast to.
        _.each(ids, function(id) {
          self.broadcast( self.room(id), self.identity, data, socketToOmit );
        });
      ...
      room: function (id, name) {
        ...
        if (!id) {
          sails.log.error('Must specify an `id` when calling `Model.room(id)`');
          return;
        }
        ...
        if (!_.isString(name)) {
          sails.log.error('Cannot create instance room name for `' + util.inspect(name) + '` (string expected).');
          return;
        }

        return 'sails_model_'+this.identity+'_'+id+':'+name;
      },
      ...
      *
      * Usage:
      * Model.subscribe(req, records [, room] )
      *
      ...
      * @param {String} room - optional room name to subscribe to (will default to model identity)
      *
      *   // Subscribe to User.update() and User.destroy()
      *   // for the specified instances (or user.save() / user.destroy()):
      *   User.subscribe(req.socket, users)
      *
      *   // Subscribe to User.publish(idInRecords, someData, 'foo')
      *   // for the specified instances:
      *   User.subscribe(req.socket, users, 'foo')
      *
      * @api public
      */
      subscribe: function (req, records, room) {
        ...
        // Records is required -- use `.watch()` to subscribe to the 'class room'.
        if (!records) {
          sails.log.error('`' + self.identity + '.subscribe` : missing or empty second argument `records`. API is `.subscribe(request, records [, room])`.');
          return;
        }

        // Make sure room is a string.
        room = room || this.identity;
        if (!_.isString(room)) {
          sails.log.error('Cannot subscribe to instance room name for `' + util.inspect(room) + '` (string expected for room name).');
          return;
        }
        ...
        _.each(ids,function (id) {
          ...
          if (sails.sockets.join( socket, self.room(id, room) )) {
            sails.log.silly(
              'Subscribed to the ' +
              self.globalId + ' with id=' + id + '\t(room :: ' + self.room(id, room) + ')'
            );
          }
        });
      },
       ...
       *
       * Usage:
       * Model.unsubscribe(req, records [, room] )
       *
       * @param {Request|Socket} req - request containing the socket to unsubscribe, or the socket itself
       * @param {Object|Array|String|Finite} records - id, array of ids, model, or array of records
       * @param {String} room - optional room name to unsubscribe from (will default to model identity)
       */

      unsubscribe: function (req, records, room) {
        ...
        if (!records) {
          sails.log.error('`' + self.identity + '.unsubscribe` : missing or empty second argument `records`. API is `.subscribe(request, records [, room])`.');
          return;
        }
        ...
        _.each(ids,function (id) {
          ...
          if (sails.sockets.leave( socket, self.room(id, room))) {
            sails.log.silly(
      ...
      _introduce: function(model) {
        ...
        sails.sockets.addRoomMembersToRooms(self._classRoom(), self.room(id) );
      ...
      _retire: function(model) {
        ...
        sails.sockets.removeRoomMembersFromRooms(self._classRoom(), self.room(id) );
      }
