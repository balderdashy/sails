...
module.exports = function(sails) {
	...
	function getPubsubMethods () {
		...
		return {
			...
			publish: function (models, messageName, data, req) {
				var self = this;

				if (typeof messageName === 'object') {
					req = data;
					data = messageName;
					messageName = null;
				}
				...
				if (!messageName) {
					messageName = sails.sockets.DEFAULT_EVENT_NAME;
				}
				...
				if (!models) {
					...
					sails.log.silly('Published ', messageName, ' to ', self.classRoom());
					sails.sockets.broadcast( self.classRoom(), messageName, data, socketToOmit );
					return;
				}
				...
				// Otherwise publish to each instance room
					...
					_.each(ids,function eachInstance (id) {
						...
						sails.log.silly("Published ", messageName, " to ", room);
						sails.sockets.broadcast( room, messageName, data, socketToOmit );
					});
				}
			...
			subscribers: function (id) {
				return sails.sockets.subscribers(this.room(id));
			},
			...
			*/
			...
			publishUpdate: function (id, changes, req, noReverse) {

				// Ensure that we're working with a clean, unencumbered object
				changes = _.clone(changes);

				var previous;
				if (sails.util.isObject(changes.__previous__)) {
					previous = changes.__previous__;
					delete changes.__previous__;
				}
					...
					_.each(changes, function(val, key) {
						...
						if (referencedModel) {
							...
							if (reverseAssociation.type == 'collection') {
								if (previous[key]) {
									ReferencedModel.publishRemove(previous[key], reverseAssociation.alias, id);
								}
								if (val) {
			...
			publishDestroy: function (id, req, previous) {
					...
					_.each(this.associations, function(association) {
						...
						if (association.type == 'model' && [association.alias]) {
							...
							if (reverseAssociation.type == 'model') {
								...
								ReferencedModel.publishUpdate(previous[association.alias].id, pubData, req, true);
							} 
							// If it's a to-many, publish a "removed" alert
							else {
								ReferencedModel.publishRemove(previous[association.alias].id, reverseAssociation.alias, id, req, true);
							}
						}
							...
							_.each(previous[association.alias], function(associatedModel) {
								...
								if (reverseAssociation.type == 'model') {
									var pubData = {};
									pubData[reverseAssociation.alias] = null;
									ReferencedModel.publishUpdate(associatedModel.id, pubData, req, true);
								} 
								// If it's a to-many, publish a "removed" alert
								else {
									ReferencedModel.publishRemove(associatedModel.id, reverseAssociation.alias, id, req, true);
								}
							});
						}
			...
			publishAdd: function(id, alias, idAdded, req, noReverse) {
				...
				if (!noReverse) {
					...
					if (reverseAssociation) {
						...
						// other side.
						if (reverseAssociation.type == 'collection') {
							reverseModel.publishAdd(idAdded, reverseAssociation.alias, id, req, true);
						}
						...
						// Otherwise, do a publishUpdate
							...
							reverseModel.publishUpdate(idAdded, data, req, true);
						}
					}
			...
			publishRemove: function(id, alias, idRemoved, req, noReverse) {
				...
				if (!noReverse) {
					...
					if (reverseAssociation) {
						...
						// other side.
						if (reverseAssociation.type == 'collection') {
							reverseModel.publishRemove(idRemoved, reverseAssociation.alias, id, req, true);
						}
						...
						// Otherwise, do a publishUpdate
							...
							reverseModel.publishUpdate(idRemoved, data, req, true);
						}
					}
			...
			publishCreate: function(values, req) {
				...
				_.each(values, function(val, key) {
					...
					if (referencedModel) {
						...
						// other side.
						if (reverseAssociation.type == 'collection') {
							ReferencedModel.publishAdd(val, reverseAssociation.alias, values.id, req, true);
						}
						...
						// Otherwise, do a publishUpdate
							...
							ReferencedModel.publishUpdate(val, pubData, req, true);
						}
					}
				}, this);		
			...
			introduce: function(id) {
				...
				sails.log.silly('Introduced model ' + room);
