	...
	function getPubsubMethods () {
		...
		return {
			...
			publish: function (models, message, data, req) {
				...
				var socketToOmit = req.socket ? req.socket : req;
			 ...
			 */
			classRoom: function () {
				sails.log.warn('class rooms are deprecated; please use instance rooms instead (or raw sails.sockets.*() methods.)');
			...
			*/
			...
			subscribe: function (req, models, cb) {
				...
				if (!models) {
					...
					sails.sockets.join( socket, self.classRoom(), cb );
          			sails.log.silly(
			...
			unsubscribe: function (req, models, cb) {
				...
				if (!models) {
					sails.log.warn(
						'Calling unsubscribe without models is deprecated; please use instance rooms instead (or raw sails.sockets.*() methods.).'
					);
					return sails.sockets.leave( socket, self.classRoom(), cb );
				}
			...
			publishCreate: function(values, req) {
				...
				sails.log.warn(
					'`.publishCreate()` is deprecated; as of Sails v0.10, you can `Model.subscribe()` '+
					'to an instance room of an associated model and then publish an update on that.' +
					'(or use the raw `sails.sockets.*()` methods instead.)'
				);
			...
			introduce: function(id, req) {
				sails.log.warn(
					'`.introduce()` is deprecated; as of Sails v0.10, you can `Model.subscribe()` '+
					'to an instance room of an associated model. (or use the raw `sails.sockets.*()` methods instead.)'
				);
