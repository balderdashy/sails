    ...
    loadModules: function(cb) {
      ...
      sails.modules.loadResponses(function loadedRuntimeErrorModules(err, responseDefs) {
        ...
        _.defaults(responseDefs, {
          ...
          notFound: cannotFindResponseWarning('notFound', 404),
          serverError: cannotFindResponseWarning('serverError', 500),
          forbidden: cannotFindResponseWarning('forbidden', 403),
          badRequest: cannotFindResponseWarning('badRequest', 400)
        });
      ...
      function cannotFindResponseWarning(responseName, statusCode) {
        return function _mockCustomResponse(err) {
          ...
          if (process.env.NODE_ENV === 'production') {
            return res.send(statusCode || 500, {status: 500});
          }
          return res.send(statusCode || 500, {status: statusCode});
        };
      }
    },
