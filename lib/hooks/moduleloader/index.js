 ...
 */
var pluginSystem = function(appPath, config){

  var fs        = require('fs');
  var file_path = config.appPath + '/config/plugins.js';
  var _         = require('lodash');

  if(fs.existsSync(file_path)){
    var list = require(file_path).plugins;
    // Checking that exist the plugin in node_modules
    return _.filter(list, function(plugin){
      fs.existsSync(appPath + '/node_modules/' + plugin);
    });
  }
    ...
    defaults: function(config) {
      ...
      return {
        ...
        paths: {
          ...
          // Server-Side Code
          //
          plugins: (pluginSystem(appPath, config)).map(function(pluginName) {
            return config.appPath + '/node_modules/' + pluginName;
          }),
