  ...
  return {
    ...
    loadUserConfig: function (cb) {
      ...
      async.auto({
        ...
        // TODO -- replace _.merge() with call to merge-dictionaries module?
        var config = _.merge(
          async_data['config/*'],
                ...
                if (identity === 'package.json' && modules[identity] === true) {
                  ...
                  try {
                    ...
                    if (!_.get(appPackageJson, 'dependencies.' + packageJson.name) && !_.get(appPackageJson, 'devDependencies.' + packageJson.name) && !_.get(appPackageJson, 'optionalDependencies.' + packageJson.name)) {
                      sails.log.debug('Ignoring hook `' + packageJson.name + '` because it isn\'t saved in your package.json file.');
                      sails.log.debug('(try installing it with `npm install --save ' + packageJson.name +'` or `npm install --save-dev ' + packageJson.name + '`)');
                      return;
                    }
