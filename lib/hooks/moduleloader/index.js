module.exports = function(sails) {
  ...
  var pluginHelper    = require('./plugins');
  var extensionHelper = require('./extensions');
  ...
  return {
    ...
    defaults: function(config) {
      ...
      return {
        ...
        paths: {
          ...
          // Server-Side Code
          // For `plugins` hooks
          plugins: pluginHelper.initialize(config.appPath),
          // For `extensions` hooks
          extensions: extensionHelper.initialize(config.appPath),
    ...
    configure: function() {
      ...
      _.extend(sails.config.paths, {
        ...
        // Server-Side Code
        // For `plugins` hooks
        plugins: pluginHelper.initialize(sails.config.appPath),
        // For `extensions` hooks
        extensions: extensionHelper.initialize(sails.config.appPath),
        // For `controllers` hook
    ...
    loadUserConfig: function(cb) {
      async.auto({
        ...
        'extensions/config/*': function loadOtherConfigFiles(cb) {
          var tasks = sails.config.paths.extensions.map(function(plugin) {
            return function(c){
              buildDictionary.aggregate({
                dirname     : plugin + '/config',
                exclude     : ['locales', 'local.js', 'local.json', 'local.coffee', 'local.litcoffee'],
    ...
    loadControllers: function(cb) {
      async.parallel({
        ...
        extensions: function(callback) {
          var tasks = sails.config.paths.extensions.map(function(m) {
            return function(c) {
              buildDictionary.optional({
                dirname            : m + '/api/controllers',
                filter             : /(.+)Controller\.(js|coffee|litcoffee)$/,
     ...
     */
     loadUserHooks: function(cb) {
       // TODO: At this point of the code, sails.config.paths.plugins contains
