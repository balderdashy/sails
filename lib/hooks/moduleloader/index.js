        ...
        try {
          ...
          _.extend(hooks, _.reduce(results.nodeModulesFolder, function(memo, module, identity) {
            ...
            if (module['package.json'] && module['package.json'].sails && module['package.json'].sails.isHook) {
              ...
              if (sails.config.installedHooks && sails.config.installedHooks[identity] && sails.config.installedHooks[identity].identity) {
                hookName = sails.config.installedHooks[identity].identity;
              }
              ...
              if (sails.hooks[hookName] || hooks[hookName]) {
                var err = new Error("Tried to load hook `" + hookName + "`, but a hook with that name already exists.");
