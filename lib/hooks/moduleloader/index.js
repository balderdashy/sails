  ...
  return {
      ...
      async.auto({
        ...
        'config/*': function loadOtherConfigFiles (cb) {
          ...
          buildDictionary.aggregate({
            ...
            filter    : /(.+)\.(js|json|coffee|litcoffee|iced|liticed)$/,
        ...
        'config/local' : function loadLocalOverrideFile (cb) {
          buildDictionary.aggregate({
            dirname   : sails.config.paths.config || sails.config.appPath + '/config',
            filter    : /local\.(js|json|coffee|litcoffee|iced|liticed)$/,
            identity  : false
          }, cb);
        },
        ...
        'config/env/*' : ['config/local', function loadEnvConfigFile (cb, async_data) {
          ...
          buildDictionary.aggregate({
            ...
            filter    : new RegExp(env + '.(js|json|coffee|litcoffee|iced|liticed)$'),
            optional  : true,
            flattenDirectories: !(sails.config.dontFlattenConfig),
            identity  : false
          }, cb);
    ...
    loadControllers: function (cb) {
      ...
      buildDictionary.optional({
        ...
        filter: /(.+)Controller\.(js|coffee|litcoffee|iced|liticed)$/,
    ...
    loadAdapters: function (cb) {
      buildDictionary.optional({
        ...
        filter    : /(.+Adapter)\.(js|coffee|litcoffee|iced|liticed)$/,
        replaceExpr : /Adapter/,
        flattenDirectories: true
      }, bindToSails(cb));
    },
    ...
    loadModels: function (cb) {
      ...
      buildDictionary.optional({
        ...
        filter    : /^([^.]+)\.(js|coffee|litcoffee|iced|liticed)$/,
        replaceExpr : /^.*\//,
        flattenDirectories: true
      }, function(err, models) {
        if (err) {return cb(err);}
