  ...
  return {
    ...
    defaults: function (config) {
        ...
        listeners: {
          file: function (root, fileStats, next) {
            if (fileStats.name.match(/(.+)\.(coffee|litcoffee)$/)) {
              ...
              hasIced = true;
            } else if (fileStats.name.match(/(.+)\.(ls)$/)) {
              hasLive = true;
            }
            next();
          },
      ...
      if (hasIced) {
        try {
          ...
          try {
            ...
            sails.log.error('Please run `npm install iced-coffee-script` to use IcedCoffeeScript!');
      ...
      if (hasCoffee) {
        try {
          ...
          try {
            ...
            sails.log.error('Please run `npm install coffee-script` to use CoffeeScript!');
            sails.log.silly('Here\'s the require error(s): ',e0,e1);
          }
        }
      }
      if (hasLive) {
        try {
          ...
          try {
            require(path.join(localConfig.appPath, 'node_modules/LiveScript/index'));
          }
          catch (e1) {
            sails.log.error('Please run `npm install LiveScript` to use LiveScript!');
            sails.log.silly('Here\'s the require error(s): ',e0,e1);
          }
        }
      }
    ...
    initialize: function(cb) {
      ...
      sails.config.moduleloader = {
        configExt: ['js','coffee','litcoffee','iced','liticed','ls','json'],
        sourceExt: ['js','coffee','litcoffee','iced','liticed','ls']
      }
