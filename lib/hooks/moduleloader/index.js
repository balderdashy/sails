  ...
  return {
    ...
    loadUserHooks: function (cb) {
      ...
      async.auto({
        ...
        // Delete hooks that are disabled and sort them according to an execution 
        // order specified in config/locals.js :
        //   userHooks: { 
        //     foo: true,
        //     bar: false,
        //   }
        if(sails.config.userHooks){
          hooks = _.reduce(hooks, function(memo, module, identity) {
            if(! (identity in sails.config.userHooks)){
              sails.log.warn('Hook `' + identity + '` bas been ignored.' +
              ' It\'s has to be present `sails.config.userHooks`,' +
              ' so please add it in your config/locals.js.');
            }else if (sails.config.userHooks[identity] !== false) {
              memo[identity] = module;
            }
            return memo;
          }, {});
          // Sort hooks according to execution order specified in config/locals.js
          var sortedHooks = {};
          _.each(sails.config.userHooks,function(module,identity){
            if(! (identity in hooks)){
              sails.log.warn('Unable to locate hook `' + identity + '` configured in `sails.config.userHooks`.');
            }else{
              sortedHooks[identity] = hooks[identity];
            }
          });
          hooks = sortedHooks;
        }
