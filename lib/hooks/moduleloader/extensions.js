...
module.exports = {
  ...
  /**
   * check paths and return an array of services available for
   * register in the core
   * @param  config sails config
   * @return array of paths
   */
  initialize: function(configAppPath){
    var file_path = configAppPath + '/config/extensions';

    // TODO: Regular expression?
    if(fs.existsSync(file_path + '.js') || fs.existsSync(file_path + '.coffee')){

      var list = _.map(require(file_path).extensions, function(plugin){
