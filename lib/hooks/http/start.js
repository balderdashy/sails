  ...
  return function startServer (cb) {
    ...
    async.auto({
      ...
      start: function (next) {
        ...
        console.log('Trying to listen on:',sails.config.port, explicitHost);
      ...
      verify: ['start', function (next) {
        ...
        if( !explicitHost && !sails.hooks.http.server.address() ) {
          ...
          // For a more graceful shutdown, we should instead consider:
          // return next(new Error(portBusyError));
        }
