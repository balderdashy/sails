		...
		if (cluster.isMaster) {
			cluster
				.on('exit', function workerExited(worker, code, signal) {
					sails.log.debug(('Worker stopped (pid: ' + worker.process.pid + ')').grey);
					cluster.fork(process.env);
				})
				.on('online', function workerOnline(worker) {
					sails.log.debug(('Worker started (pid: ' + worker.process.pid + ')').grey);
				});
