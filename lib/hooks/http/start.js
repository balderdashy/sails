module.exports = function (sails) {
	...
	var $getServer = cluster._getServer;
	...
	return function startServer (cb) {
		...
		if (cluster.isMaster && starter === 'sails') {
			cluster
				.on('exit', function workerExited(worker, code, signal) {
					sails.log.debug(('Worker exited (pid: ' + worker.process.pid + ')').grey);
					cluster.fork();
				})
				.on('listening', function workerOnline(worker) {
					sails.log.debug(('New Worker forked (pid: ' + worker.process.pid + ')').grey);
				});
