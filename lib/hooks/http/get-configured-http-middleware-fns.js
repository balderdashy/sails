  ...
  return _.defaults(sails.config.http.middleware || {}, {
    ...
    session: (function() {
      ...
      if (!sails.config.session || !sails.config.session.store) {
        sails.log.error('Cannot load default HTTP session middleware without `sails.config.session.store` configured!');
        return;
      }
      if (sails.config.session && sails.config.session.store) {
    ...
    //
    // * TODO: ensure we've got equivalently-nice error handling in Skipper itself
    // * (i.e. where we intercept the `next` function).  Then remove this, since it
