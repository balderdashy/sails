...
module.exports = function(sails) {
  ...
  return {
    ...
    configure: function() {
      ...
      // Path static files will be served from
      if (sails.config.paths.public) {

        // TODO: apply this intelligence to the other path configs

        // if a leading slash was provided, assume this is an absolute path
        if ( _looksLikeAnAbsolutePath(sails.config.paths.public) ) {
          sails.config.paths.public = path.resolve(sails.config.paths.public);
        }
        // otherwise, resolve it from the configured `appPath`
        else {
          sails.config.paths.public = path.resolve(sails.config.appPath, sails.config.paths.public);
        }

        // TODO: try doing something more helpful w/ stat-ing the directory and trying different strategies
      }
      else {
        sails.config.paths.public = path.join(config.appPath,'.tmp/public');
      }
 ...
 * @return {Boolean} if pathstr has a leading slash
 */
function _looksLikeAnAbsolutePath (pathstr) {
