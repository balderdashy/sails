  ...
  return function initialize(cb) {
    ...
    (function _waitForSessionHookIfApplicable(next){
      ...
      try {
        ...
        // deprecation warning.
        if (_.includes(sails.config.http.middleware.order, 'handleBodyParserError') && !_.isFunction(sails.config.http.middleware.handleBodyParserError)) {
        ...
        // a custom definition for it (i.e. it's trying to use the default) log a
        // deprecation warning.
        if (_.includes(sails.config.http.middleware.order, 'methodOverride') && !_.isFunction(sails.config.http.middleware.methodOverride)) {
        ...
        var removedHttpMiddleware = _.remove(sails.config.http.middleware.order, function(middleware) {
          return _.includes(['handleBodyParserError', 'startRequestTimer', '404', '500'], middleware);
        });

        // Warn about an explicit `startRequestTimer` in the order, or a custom middleware implementation of it.
        if (_.includes(removedHttpMiddleware, 'startRequestTimer') || sails.config.http.middleware.startRequestTimer) {
        ...
        if (_.includes(removedHttpMiddleware, '404') || sails.config.http.middleware['404']) {
        ...
        if (_.includes(removedHttpMiddleware, '500') || sails.config.http.middleware['500']) {
