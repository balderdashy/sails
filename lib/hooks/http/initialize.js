  ...
  return function initialize(cb) {
    ...
    (function _waitForSessionHookIfApplicable(next){
      ...
      try {
        ...
        sails.on('router:reset', function(routeToRemove) {
        ...
        // TODO: do startRequestTimer here
        ...
        var removedHttpMiddleware = _.remove(sails.config.http.middleware.order, function(middleware) {
          return _.contains(['startRequestTimer', '404', '500'], middleware);
        });
        _.each(removedHttpMiddleware, function (dupe) {
          sails.log.debug('The `' + dupe + '` middleware is added to your app automatically in Sails 1.0.');
          // If the middleware is being overridden, reassure the end user that that's okay.
          if (sails.config.http.middleware[dupe]) {
            sails.log.debug('You can still override the middleware, you just don\'t need to add it to');
            sails.log.debug('the `sails.config.http.middleware.order` list.)\n');
          }
          // Otherwise just note that the entry in the list is being ignored.
          else {
            sails.log.debug('(ignoring entry from the `sails.config.http.middleware.order` list.\n');
          }
        });
        ...
        var preRouterMiddleware = ['startRequestTimer'];//TODO: instead, just app.use this before dealing w/ config
        ...
        // TODO: just manually app.use this after processing middleware instead
        ...
        sails.once('ready', function addPostRouterMiddleware() {
          ...
          // TODO: put 404 and 500 handlers here
