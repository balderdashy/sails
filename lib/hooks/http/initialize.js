...
module.exports = function(sails) {
  ...
  return function initialize(cb) {
    ...
    (function _waitForSessionHookIfApplicable(next){
      ...
      // TODO: consider changing the above to ".once()"
      // (don't want to do that before 1.0 to avoid any compatibility issues)
      ...
      // middleware functions in the configured order (`sails.config.http.midleware.order`).
        ...
        _.each(sails.config.http.middleware.order, function (middlewareKey) {
          ...
          // If this is `$custom` then for compatibility, if `sails.config.http.customMiddleware`
          // was provided, we'll call it.  This is handled above.
          ...
          // If a middleware fn by this name is not configured, then skip this key.
          if (!referencedMwr) {
            sails.log.verbose('`sails.config.http.middleware.order` references an unrecognized middleware function-- that is, it was not provided as a key in the `sails.config.http.middleware` dictionary.');
            return;
          }
