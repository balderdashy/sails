    ...
    session: (function() {
      if (sails.config.session && sails.config.session.store) {
        ...
        return function (req, res, next){
          ...
          // If configured to do so, only run session middleware for certain routes.
          if (sails.config.session.routesDisabled) {

            // Parse configured route addresses.
            //================================================================================
            // TODO: move this code out elsewhere so it only has to run once.
            sails.hooks.session.disabledRoutes = _.reduce(sails.config.session.routesDisabled, function (memo, routeAddressStr){
              var addrPieces = routeAddressStr.split(' ');
              var method;
              var urlPattern;
              if (addrPieces.length === 1) {
                ...
                return next(new Error('Invalid `routesDisabled` config: If specified, must be an array of normal route address strings.'));
              }

              // TODO: use path2regexp(urlPattern);
              var urlPatternRegExp = /todo/;
            ...
            // http://sailsjs.org/documentation/reference/request-req/req-path
            // http://sailsjs.org/documentation/reference/request-req/req-method
            var isSessionDisabled = _.any(sails.hooks.session.disabledRoutes, function (disabledRouteInfo){
          ...
          }//</if `config.session.routesDisabled` is set>

          // Then run the session middleware for this HTTP request.
          // (attaches req.session, etc.)
