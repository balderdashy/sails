...
module.exports = function(sails, app) {
  ...
  return _.defaults(sails.config.http.middleware || {}, {
    ...
    /**
     * Adds logging, via the morgan package, to each HTTP request,
     * showing useful information such as the response code and the response time.
     *
     * As a default the logging is turned on in the development environemnt
     * and turned off in productin environment
     */
    requestLogger: (function() {
      var loggerSettings = sails.config.http.requestLogger;
      if (IS_PRODUCTION && loggerSettings.inProduction === true) {
        return require('morgan')(loggerSettings.format);
      } else if (!IS_PRODUCTION && loggerSettings.inDevelopment === true){
        return require('morgan')(loggerSettings.format);
      }
    })(),
