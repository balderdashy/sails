...
module.exports = function(sails, app) {
  ...
  return _.defaults(sails.config.http.middleware || {}, {
     ...
     */
    requestTimer: !IS_PRODUCTION && function startRequestTimer(req, res, next) {
      var _startTime = process.hrtime();
      req.on('end', function() {
        var diff = process.hrtime(_startTime);
        ...
        sails.log.verbose(req.ip + "'s " + type + " request to", req.method, req.url, 'took ' + val + 'ms');
      });
      next();
    },
