module.exports = function (sails) {
	...
	function Hook ( definition) {
		...
		// Ensure that the required hierarchy exists before merging definitions
		normalize( this );
		...
		this.load = function (cb) {
			...
			this.initialize && this.initialize(cb);

			// Bind routes before any of the static app routes
			sails.on('routes:before', function () {
			...
			sails.on('routes:after', function () {
		...
		/**
		 * Ensure that a hook definition has the required hierarchy
		 */
