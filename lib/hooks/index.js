  ...
  function Hook(definition) {
    ...
    this.load = function(cb) {
      ...
      function applyDefaults() {
        ...
        var defaults = (_.isFunction(self.defaults) ?
                  self.defaults(sails.config) :
                  self.defaults) || {};
        // Replace the special __configKey__ key with the actual config key
        if (self.defaults.__configKey__ && self.configKey) {
          self.defaults[self.configKey] = self.defaults.__configKey__;
          delete self.defaults.__configKey__;
        }
