    ...
    try {
      ...
      _.each(helperDefs, function(helperDef, identity) {
        try {
          ...
          // Use filename-derived `identity` if no explicit identity was set,
          // but exclude any extra hierarchy.
          // (Otherwise, as of machine@v15, this could fail with an ImplementationError.)
          if (!helperDef.identity) {
            helperDef.identity = _.last(identity.split('/'));
          }
