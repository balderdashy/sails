    ...
    configure: function() {
      ...
      if (sails.config.helpers.usageOpts.arginStyle !== 'named' || sails.config.helpers.usageOpts.execStyle !== 'deferred') {
        ...
        if (isMinSVRPointedAtSensitiveV1Prerelease) {
          ...
          sails.log.warn(
            'Based on its package.json file, it looks like this app was generated using the Sails beta, '+
            'but with a version prior to v1.0.0-44.  (It depends on sails@'+localSailsSVR+'.)  '+
            'But in the 1.0.0-44 prerelease, a change was introduced that makes helpers support standard, '+
            'serial usage like `.changePassword(\'abc123\'), rather than always being forced to use named '+
            'parameters like `.changePassword({password:\'abc123\'})`.\n'+
            'To avoid breaking this app, backwards-compatible settings have been set automatically for '+
            'you this time to keep your helpers compatible.  (But be sure to resolve this as soon as possible, '+
            'or you might lift one day soon and realize your helpers don\'t work!)\n'+
            'To resolve this, either:\n'+
            ' • (A) configure `sails.config.helpers.usageOpts.arginStyle` as "named" and also '+
            '`sails.config.helpers.usageOpts.execStyle` as "deferred"`, which will make everything '+
            'work the way it did before.\n'+
            ' • or (B) change any code in this app that invokes helpers.  You\'ll need to either take '+
            'advantage of serial usage in your app, configure the aforementioned usage opts, or tack on '+
            '`.with()` every time you call a helper to continue using named parameters.  Then upgrade to '+
            'the latest version of Sails v1.0 and update your package.json file accordingly.\n'+
            '(This warning will continue to display until this has been solved one way or the other.)\n'+
