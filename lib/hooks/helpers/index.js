    ...
    defaults: {
      helpers: {
        ...
        usageOpts: {
          arginStyle: 'serial',
          execStyle: 'natural',
          implementationSniffingTactic: 'analog'
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          // ^FUTURE: in 'analogOrClassical' as a default (probably explicitly
          // in the generated boilerplate config rather than as an implicit
          // default-- but not necessarily.  In the mean time, it's available as
          // an opt-in setting.  One must create the config/helpers.js file oneself
          // of course, or configure it some other way.)
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        },
    ...
    furnishHelper: function(identityPlusMaybeSlug, nmDef){
      ...
      if (!_.get(sails.helpers, fullKeyPath)) {
        ...
        if (slug) {
          ...
          sails.helpers[machine.getMethodName(identity)] = machine.buildWithCustomUsage(_.extend({ def: nmDef }, sails.config.helpers.usageOpts));
        }
