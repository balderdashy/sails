      ...
      if (sails.config.helpers.usageOpts.arginStyle !== 'named' || sails.config.helpers.usageOpts.execStyle !== 'deferred') {
        ...
        if (isMinSVRPointedAtSensitiveV1Prerelease) {
          ...
          sails.log.warn(
            ...
            'But in the 1.0.0-44" prerelease, a change was introduced that makes helpers invocable with '+
            'standard, serial usage like `.changePassword(\'abc123\'), rather than always being forced '+
            'to use named parameters like `.changePassword({password:\'abc123\'})`.\n'+
            ...
            'advantage of the new default usage style or tack on `.with()` to each call to continue using '+
            'named parameters.  Then upgrade to the latest version of Sails v1.0 and update your package.json '+
            'file accordingly.\n'+
