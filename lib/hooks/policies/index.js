...
module.exports = function(sails) {
  ...
  var policyHookDef = {
    ...
    buildPolicyMap: function () {
      ...
      var detectedDeprecatedConfig = false;

      // Loop through the keys looking for the old-style "controller-based" policy config,
      // and if we find it then expand it out to the new-style and set a flag so that we
      // can output a warning.
      _.each(_.without(_.keys(sails.config.policies), 'moduleDefinitions'), function(key) {
        ...
        if (_.isPlainObject(sails.config.policies[key])) {
          ...
          // If so, we've got some legacy config to transform.
          detectedDeprecatedConfig = true;
      ...
      // If we detected deprecated config, log a warning including the transformed config
      // for the user to copy/paste.
      if (detectedDeprecatedConfig) {
        sails.log.debug('Using "controller-based" configuration is deprecated in Sails 1.0.');
        sails.log.debug('Please migrate your config/policy.js file to use action-based configuration.');
        sails.log.debug('(You can copy and paste the transformed config below to start)');
        sails.log.debug('-------------------------------------------------');
        sails.log.debug('\n' + util.inspect(_.omit(sails.config.policies, 'moduleDefinitions'), {depth: null}));
        sails.log.debug('-------------------------------------------------\n');
      }
