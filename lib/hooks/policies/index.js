module.exports = function(sails) {
	...
	return {
		...
		bindPolicies: function() {
			...
			sails.log('Currying policies onto controllers...');
			
			// Iterate through each controller
			_.each(sails.controllers, function(_c, id) {

				var policy = this.mapping[id];
				var actions, actionFn;
				var controller = sails.controllers[id];
				...
				if (_.isArray(policy)) {
					...
					if ( util.isDictionary(controller) ) {
						sails.log('Applying policy to controller\'s (' + id + ') actions...');
						_.each(actions, function(actionId) {
							actionFn = controller[actionId];
							sails.log('Applying policy to ' + actionId + '...');
					...
					// -> apply the policy directly
						...
						sails.log('Applying policy to top-level controller middleware fn (' + id + ')...');
				...
				// iterate through each of this controller's actions and apply policies in a way that makes sense
					...
					_.each(actions, function(actionId) {
						...
						sails.log('Applying policy to action (' + id + '.' + actionId + ')...');
			...
			sails.log('Policy-controller bindings complete!');
			sails.emit('hook:policies:bound');
			
			// console.log('CONTROLLERS:',sails.controllers);
			// console.log('POLICIES:',this.mapping);
		},
