module.exports = function(sails) {
	 ...
	 */
	var FatalError = {
		unknownPolicy: function (policy, source) {
			...
			sails.log.error('It should be located at: `' + sails.config.paths.policies + '/' + policy + '.js`');
	...
	var policyDef = {
		...
		normalizePolicy: function (policy) {
			...
			if ( util.isString(policy) ) {
				...
				if ( !this.middleware[policy] ) {
					return FatalError.unknownPolicy(policy);
				}
		...
		bindDirectlyToRoute: function (event) {
			...
			console.log(event.target, this.middleware);

			// Policy doesn't exist
			if ( !this.middleware[event.target.policy] ) {
				return FatalError.unknownPolicy(event.target.policy, 'config.routes');
			}

			// Bind policy function to route
			var policyFn = this.middleware[event.target.policy];
			sails.router.bind(event.path, policyFn, event.verb);
		}
	 ...
	 */
	sails.on('route:typeUnknown', policyDef.bindDirectlyToRoute);
