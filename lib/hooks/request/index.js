...
module.exports = function(sails) {
  ...
  return {
    ...
    initialize: function(cb) {
      ...
      // Wait for the `before` signal from the router.
      // Also wait for `responses` hook if it is enabled
      var eventsToWaitFor = ['router:before'];
      if (sails.hooks.responses) {
        eventsToWaitFor.push('hook:responses:loaded');
      }
    ...
    routes: {
      ...
      // TODO:
      // refactor function bound above to implement the declarative pattern
      // used in other hooks, i.e.:
      // before: {
      ...
      // }
      //
      // (needs tests first to make sure nothing breaks!!)
    }
  };
};
