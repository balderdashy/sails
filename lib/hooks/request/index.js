module.exports = function (sails) {
		...
		initialize: function (cb) {
			...
			sails.on('router:before', function () {
				...
				sails.router.bind('/*', function addSugarMethods (req, res, next) {
					...
					_mixinResFobidden(req,res);
					_mixinResInvalid(req,res);
	...
	function _mixinResServerError (req, res) {
		res.serverError = function respond500 (err) {
			sails.config[500](err, req,res, function fallback (){
				res.send(err, 500);
			});
		};
	}
	...
	function _mixinResNotFound (req, res) {
		res.notFound = function respond404 () {
			sails.config[404](req,res, function fallback (){
				res.send(404);
			});
		};
	}
	...
	function _mixinResForbidden (req, res) {
		res.forbidden = function respond403 (err) {
			sails.config[403](err, req,res, function fallback (){
				res.send(err, 403);
			});
		};
	}
	...
	function _mixinResInvalid (req, res) {
		res.invalid = function respond400 (err) {
			sails.config[400](err, req,res, function fallback (err) {
				res.send(err, 400);
			});
		};
	}
