module.exports = function(sails) {
	...
	function mixinSugar(req, res) {
		...
		var getAllParams = function() { return allParams; };
		...
		if (req.params.all) {
			sails.log.warn('req.params.all() could not be exposed because another module has modified the global Array prototype.  You can uninstall the other module or use `req.params.allParams()` instead.');
		} else {
			Object.defineProperty(req.params, 'all', { value: getAllParams });
		}

		// Define a new non-enumerable function: req.params.allParams()
		// (but only if an `:all` route parameter doesn't exist)
		if (!req.params.allParams) {
			Object.defineProperty(req.params, 'allParams', { value: getAllParams });
		}
