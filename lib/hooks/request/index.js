module.exports = function (sails) {
	...
	function mixinSugar (req,res) {
		...
		// Mixin route params using wild hacks
		var routeParams = util.clone(req.params) || {};
		util.each(Object.keys(req.params), function (paramName) {
			allParams[paramName] = req.params[paramName];
		});

		// Define a new non-enuerable function, req.parmas.all()
		if (!req.params.all) {
			Object.defineProperty(req.params, 'all', {
				value: function bundleAllParams () {
					return allParams;
				}
			});
		}
