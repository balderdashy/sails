...
E_INVALID_PARAMS.prototype.inspect = function () {
  var output = util.format('Invalid parameters sent for route: %s', this.route);
  output += '\n';
  output += _.map(this.invalidParams, function (param) {
    return util.format('  -> `%s` should be a "%s"', param.property, param.rule);
  });
  output += '\n';
  output += 'Proper usage:';
  output += '\n';
  output += util.format(this.usage);
  return output;
};
