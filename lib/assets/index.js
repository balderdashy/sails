...
var TemplateAsset = rack.Asset.extend({
	create: function() {
		...
		var root = process.cwd() + '/assets/';
		self.paths = search(sails.config.assets.sequence, self.regex);
		self.contents = [];
		_.each(self.paths, function(path) {
		  var name = path.substr(root.length).replace(self.regex, '');
		  var extension = (self.regex.exec(path) || ['', 'template'])[1];
		  self.contents.push('<script type="text/'+extension+'" data-name="' + name + '">\n');
		  var fileContents = fs.readFileSync(path, 'utf8');
		  self.contents.push(fileContents);
		  self.contents.push('</script>');
		});
		self.contents = self.contents.join('');
		self.emit('created');
	}
});
