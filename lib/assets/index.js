...
function prep_snippet_params (params) {
	// if given something
	if (params) {
...
var Rack = rack.Rack.extend({
	snippet: function(filename, params) {
		if (filename) {
			var filepath = get_snippet_path(filename);
			params = prep_snippet_params(params);
			var assetContent = fs.readFileSync('/DROPBOX/500_PROJECTS/904_sailscore/220_snippetsproj/views/snippets/' + filename + '.ejs', 'utf8');
			var ejs = require('ejs');
			ejs.render(file, params, function(err, result) {
				return result;
			});
		}
	},
