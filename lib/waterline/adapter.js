...
module.exports = function(adapterDef, cb) {
	...
	// Streaming
	//////////////////////////////////////////
	this.stream = function (collectionName, iterator, prefix, suffix) {
		var usage = _.str.capitalize(this.identity) + '.stream(iterator, [prefix], [suffix])';
		if (this.terminated) usageError('Chain is already terminated!');

		var StreamInterceptor = require('./streamInterceptor');

		var dam = new StreamInterceptor({
			prefix: prefix,
			suffix: suffix
		});
