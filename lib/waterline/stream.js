...
sails.transformations = {
	...
	json: {
		...
		write: function (model, index, cb) {
			...
			// Prefix with opening [
			if (index === 0) transformedModel = '[' + transformedModel;

			// Prefix with comma (except the first time)
			else if (index > 0) transformedModel = ',' + transformedModel;
...
var ModelStream = function(transformation) {
	...
	this.transformation = transformation || sails.transformations.json;
...
ModelStream.prototype.end = function(err) {
	var self = this;
