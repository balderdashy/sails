...
module.exports = function (options,cb) {
	...
	// Begin long thread of serial execution
	async.series([

		function (cb) {
			async.each(_.keys(adapters), instantiateAdapter, cb);
		}, 
		...
		function (cb) {
			async.each(_.keys(adapters), function (adapterName, cb) {
		...
		function (cb) {
			async.each(_.keys(collections), instantiateCollection, cb);
		},

		// Provide hook for process-end event and teardown waterline
		function (cb) {
			listenForProcessEnd(cb);
		}
	...
	function teardown (cb) {
		...
		async.each(_.values(adapters),function (adapter,cb) {
			adapter.teardown(cb);
		},cb);
	}
