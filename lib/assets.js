...
var JavascriptAsset = rack.Asset.extend({
    ...
    create: function() {
        var self = this;
        self.paths = search(sails.config.rigging.sequence, /\.js$/);
        self.assets = []
        async.forEachSeries(self.paths, function(path, next) {
            ...
            self.contents = uglify.minify(self.contents, {fromString: true}).code;
            self.isDev = false;
            self.emit('created');
        });
    }
...
module.exports = new Rack([
    new JavascriptAsset({url: '/app.js'}),
    new CssAsset({url: '/style.css'}),
