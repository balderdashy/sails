...
fileWatcher.isReady = true;
fileWatcher.on('newListener', function(event, callback) {
    if (event == 'ready') {
        console.log('on new listener');
        console.log(fileWatcher.isReady);
        if (fileWatcher.isReady == 'middle') {
            console.log("hey middle time");
            fileWatcher.isReady = true;
            fileWatcher.removeListener(callback);
            return 
        }
...
var count = 0;
handle = rack.Rack.prototype.handle
var Rack = rack.Rack.extend({
    handle: function() {
        ...
        if (isProduction) {
            ...
            fileWatcher.once('ready', function(assets) {
                console.log(count);
                if (fileWatcher.isReady == 'middle') {
...
var watchFiles = function() {
    _.each(sails.config.assets.sequence, function(dirname) {    
        fs.watch(dirname, function() {
            console.log('we gotz wadged');
            assets = createAssets();
            fileWatcher.isReady = false;
            assets.once('complete', function() {
                fileWatcher.isReady = 'middle';
                console.log(fileWatcher.isReady);
                fileWatcher.emit('ready', assets.assets); 
            });
        });     
    });
};
