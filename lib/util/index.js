...
exports.normalizeErrors = function normalizeErrors(errOrErrs) {
	...
	if ( !_.isArray(errOrErrs) ) {
		errorsToDisplay = [errOrErrs];
	}

	// Ensure that each error is formatted correctly
	return _.map(errorsToDisplay, function (errorsToDisplay, i) {
		var e;

		// Make error easier to read, and normalize its type
		if (errorsToDisplay[i] instanceof Error) {
			return e;
		}
		else {
			e = nodeutil.inspect(errorsToDisplay[i]);
			console.log(e);
			errorsToDisplay[i] = new Error(e);
		}
	});
	// errorsToDisplay[i] = {
	// 	message: errorsToDisplay[i].message,
	// 	stack: errorsToDisplay[i].stack
	// };
};
