	...
	_.each(config.rigging.sequence, function(item,index) {
		try {
			...
			if(!inode.isDirectory()) {
				sails.log.warn("Ignoring: "+item+" (files are not allowed in rigging.sequence in development mode-- only directories.)  Please check your configuration.");
				config.rigging.sequence.splice(index,1);
			}

		} catch(e) {
			sails.log.warn("Ignoring: "+item+" (listed in rigging.sequence but does not exist.)  Please check your configuration.");
			// console.warn("Ignoring: "+item+" (listed in rigging.sequence but does not exist.)  Please check your configuration.");
			config.rigging.sequence.splice(index,1);
		}
	});
