...
exports.validate = function (config, originalUserConfig) {
	...
	var legacyAdapterMappings = {
		'waterline-dirty': 'sails-dirty',
		'waterline-mysql': 'sails-mysql'
	};
	_.each(sails.config.adapters, function (def, label) {
		_.each(legacyAdapterMappings, function (newModuleName, oldModuleName) {
			var warning = log.warn("The '"+oldModuleName+"' adapter module is now called '"+newModuleName+"'.  Please update your config and run npm install.");
			if (def === oldModuleName) {
				sails.config.adapters[label] = newModuleName;
				log.warn(warning);
			}
			else if (def && (def.module === oldModuleName)) {
				sails.config.adapters[label].module = newModuleName;
				log.warn(warning);
			}
		});
	});
