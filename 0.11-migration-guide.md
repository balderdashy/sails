...
There aren't many changes that will affect how you use Sails in your project, but there are a few important changes to be aware of.  We've listed them below.
...
+ `http`       _(which provides an HTTP server)
+ and 16 others.
...
In the process of pulling the `sockets` hook _out_ of core, the logic which interprets requests has been normalized and is now located _in_ Sails core.  As a result, the `sails.request()` method is now much more powerful.
