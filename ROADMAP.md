...
Support multi `assets pipeline` | [@hellowin](https://github.com/hellowin) | Able to define specific linker like `<!--SCRIPTS--ADMIN--><!--SCRIPTS--ADMIN END-->` to compile any assets from `./assets-admin`. So we can have different assets for different front end, like `app-assets`, `admin-assets`, etc.
