...
Allow setting the path of sails.sid session cookie | The cookie path of the sails.sid session cookie is by default set to / but some applications run in different folders rather than sub domains (Company policies). This way the cookie interfears with other applications running on the same domain. Ofcourse you can make a deal with your colleages to use different cookie names but we also would like to restrict the session cookie access to a single sub directory. This is only possible if sails allows in the configuration to set the path of the session cookie.
