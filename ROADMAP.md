...
_(feel free to suggest things)_
 ...
 create `afterRead` on the models lifecycle | [@cburatto](https://github.com/cburatto)| I might want to make changes (like strip elements) from an object before serving it -- for example, strip passwords from user data. Currently, you can do this on `afterCreate` ond `afterUpdate`, and the only way to do this on regular reads is to create your own Util and remember to add it whenever you need. With `afterRead` I could write my "strip" code and always be safe it is done. We could also have a flag called "skipAfterCreate" for the scenarios where you wouldn't like it to run.
